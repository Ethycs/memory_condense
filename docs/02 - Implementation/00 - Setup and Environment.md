...
4. **tiktoken fetches `cl100k_base` on first use** — needs network once, then cached. `cl100k_base` is the single local tokenizer proxy for budgeting, regardless of which LLM actually runs. Benchmark chat prompts add an explicit fixed framing reserve and bind the vocabulary digest in the report. This remains a deterministic local proxy, not the provider's tokenizer; nonzero provider-reported input usage is checked separately.
