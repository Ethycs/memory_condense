...
`candidate_path_handoffs_not_capability_safe`. The activation audit confirmed
that the 300-row treatment/order/query/corpus/provider/linker/factory joins are
complete, and source certification now requires the on-disk case-folded `.py`
source membership under `src/memory_condense` and `tools` to equal the tracked
case-folded `.py` membership exactly. An ignored `.py` source file can no longer
survive any certification point while the receipt names clean `HEAD`; this is
a source-membership check, not a held source-tree snapshot or a complete Python
import closure.

The remaining blocker is publisher *composition*. The execution workspace and
outer candidate staging APIs expose pathnames to independently owned inner
publishers. A rename-and-replace after that exposure can make the inner
publisher write a complete tree into the replacement before the outer held
identity detects the swap. Outer rollback then correctly refuses to delete the
unissued replacement, but confidential scratch or a false-only package has
already been written outside the issued object. The generic corpus publisher's
`OwnedCorpusDirectory` is also a copyable value rather than an opaque registry
capability and does not inventory file identities at creation, so its rollback
authority can delete an allowed-name file it did not issue. Its generic
snapshot verifier also accepts externally hard-linked file identities. Outer
candidate file creation has create-before-registration windows, and candidate
promotion has an atomic rename-before-registry transition that can strand a
public target if the transition fails. The pinned treatment loader reads the
whole input before any size bound or digest rejection. Finally, BGE/Qwen
verification and later model loading reopen checkpoint paths, so
replace/load/restore can make executed bytes differ from the declared
checkpoint hashes. All execution and Qwen-attestation fields therefore remain
literal false. Ignored bytecode and native-extension import artifacts are also
not covered by the `.py` membership projection; a future activation boundary
must reject or independently authenticate them before claiming that committed
source is the code CPython executed.

Base store, query-input, and derived-package publication and the writable
derived lifecycle remain capability-safe individually: held identities, an
atomic one-shot claim, descriptor-bound index loading, and atomic finalization
protect those boundaries. An owned open accepts only the exact current-process
unfinalized clone registration and deserializes at most 512 MiB of held SQLite
bytes into memory. The original file remains visible until close publication
begins, and successful close proves the sealed replacement. A failed close
quarantines an unfinalizable clone whose database may already be partially or
fully changed. A crash before publication can lose in-memory mutations while
leaving the durable one-shot claim, so an unfinalized reconstructed clone is
deliberately not reopenable. Finalized stores remain registry-free and are
verified/replayed from one held base and derived snapshot. The private
source-aware candidate path remains static-audit plumbing only. False-only
...
schemas, but they must not generate or freeze a production corpus. Before a
genuine run, all D0 `src` work must also be committed, because the current
route/corpus implementation
...
2. The v2 corpus requires exactly 200 development and 100 validation rows in
   ...
   `candidate_path_handoffs_not_capability_safe`. The activation audit found
   that exposed workspace/staging paths do not compose the individually held
   inner publishers across rename-and-replace, generic rollback lacks an
   opaque exact-file-inventory capability and accepts hard-linked generic
   files, outer publication has create/rename-to-registry gaps, the treatment
   read is unbounded, and checkpoint verify/load remains pathname-ABA
   susceptible. Exact tracked `.py` source membership is now certified, while
   ignored bytecode/native import closure remains open. The row/source/runtime
   joins passed provider-free audit, but those facts do not open the runner.
   Base store, query, derived publication, writable
   open/lease/close/finalization, and registry-free finalized replay continue
   to use capability-authorized held identities. Owned SQLite publication
   begins at close and is bounded to 512 MiB; successful close proves the
   sealed image, while failure quarantines an unfinalizable clone whose file
   may have changed. Abandoned claims remain one-shot. Real candidate
   generation remains blocked until identity-preserving composition and held
   checkpoint loading are implemented and audited.
