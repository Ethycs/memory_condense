...
`candidate_execution_activation_not_audited`. Base store, query-input, and
derived-package publication and the writable derived lifecycle now use
held-identity capabilities, an atomic one-shot claim, descriptor-bound index
loading, and atomic finalization. An owned open accepts only the exact
current-process unfinalized clone registration, deserializes at most 512 MiB of
held SQLite bytes into memory. The original file remains visible until close
publication begins, and successful close proves the sealed replacement. A
failed close quarantines an unfinalizable clone whose database may already be
partially or fully changed. A crash before publication can lose in-memory
mutations while leaving the durable one-shot claim, so an
unfinalized reconstructed clone is deliberately not reopenable. Finalized
stores remain registry-free and are verified/replayed from one held base and
derived snapshot. Candidate execution activation itself has not yet received
the separate audit required to open the public launcher. The private
source-aware projection is retained only for static audit and does not certify
an execution. False-only
...
2. The v2 corpus requires exactly 200 development and 100 validation rows in
   ...
   `candidate_execution_activation_not_audited`. Base store, query, derived
   publication, writable open/lease/close/finalization, and registry-free
   finalized replay now use capability-authorized held identities. Owned
   SQLite publication begins at close and is bounded to 512 MiB; successful
   close proves the sealed image, while failure quarantines an unfinalizable
   clone whose file may have changed. Abandoned claims remain one-shot. Real candidate
   generation remains blocked until public activation receives its own audit.
