...
| `00 - Theory/05 - EM-LLM Episodic Discourse Closure for Diffuse Retrieval.md` | **provider-free prototype / unmeasured** | Implements general source-grounded episodes, discourse obligations, bounded closure, and atomic full-prompt packing; EM-style autoregressive surprise is optional, and built-in paths persist zero request-token state |
...
- "How will diffuse evidence use EM-LLM?" → `00 - Theory/05 - EM-LLM Episodic Discourse Closure for Diffuse Retrieval.md` — the general closure mechanics are implemented provider-free; surprise-refined boundaries remain an optional hypothesis to test against fixed and embedding-change controls.
