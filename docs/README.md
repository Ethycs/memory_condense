...
**Date**: 2026-08-22 (adds the live fixed-stage S1 development diagnostic)
...
> **Fixed-stage campaign status.** The 8,000/256-token fixed-S1 path has now completed a live diagnostic on the original ten development questions: exactly 10 physical Terra calls and 10 physical Sol calls, zero retries, byte-identical no-call replay, and 9/10 independent semantic accuracy. The sole miss rendered the current value as `Close to 1300` rather than the gold `1300`. The formal result remains `insufficient_population`: this is not the locked 100-question validation run, it does not meet >=95%, and Mem0 remains unrun. See Research Log 27 for the measured artifacts and Research Log 26 for the 100Q protocol.
...
| `10 - Research Log/27 - 2026-08-22 - Fixed-stage S1 LiteLLM development diagnostic.md` | **live fixed-stage development diagnostic / formal gate still open** | The historical ten-question 1M retrieval made 10 fixed-S1 Terra calls and 10 independent Sol calls with zero retries, scored 9/10, and replayed byte-identically with no new calls; the sole miss was approximate `Close to 1300`, the gate correctly reports `insufficient_population`, and neither the 100Q validation result nor Mem0 exists yet |
...
- "Did the new fixed-stage answer and judge paths work live before the 100Q run?" → `10 - Research Log/27 - 2026-08-22 - Fixed-stage S1 LiteLLM development diagnostic.md` — yes as a ten-question development diagnostic: 10 Terra plus 10 Sol physical calls, no retries, 9/10 independent semantic accuracy, transparent preservation of the sandbox-blocked request-only attempt, and byte-identical zero-call replay. It is not the formal validation result, >=95% was not met, and Mem0 remains unrun.
