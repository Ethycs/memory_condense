"""Rebind parent-budgeted native trees while preserving all summary and raw fields."""
import json

from memory_condense.domain._discourse_identity import identity_sha256
from memory_condense.domain._tokenizer import count_tokens
from memory_condense.search.episodes.user_spine_hierarchy import _render_channels
from memory_condense.search.native_hierarchy_occurrence import NativeHierarchyBinding
from memory_condense.search.section_routing import SectionSummaryIndex
from memory_condense.search.section_summary import SectionSummary


def parent_summary_channels(section):
    """Decode the explicit 512-token parent contract without loosening old readers."""
    ...
    if type(body) is not dict or set(body) != {
        ...
        raise ValueError("parent-budgeted hierarchy requires explicit summary channels")
    ...
    if any(value is not None and (type(value) is not str or not value.strip()
                                or count_tokens(value) > 512) for value in values):
        raise ValueError("parent-budgeted summary exceeds the 512-token channel contract")
    if section.summary != _render_channels(*values, section.spans):
        raise ValueError("parent-budgeted summary dates or channel encoding changed")
    if (values[0] is not None) != any(span.role == "user" for span in section.spans):
        raise ValueError("parent-budgeted user-spine summary attribution changed")
    return values


def bind_parent_budget_hierarchy(template, source, atoms):
    ...
    for old in original.sections:
        users, attached = parent_summary_channels(old)
