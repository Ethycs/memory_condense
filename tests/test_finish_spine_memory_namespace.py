...
from tools import finish_spine_memory_namespace as worker
...
def fixture(tmp_path, monkeypatch, *, invalid=False, schema=False, raw_ready=True, wrong_admission=False):
    ...
    binding = {"offset": 60, "request_count": 792, "corpus_root": str(corpus),
    ...
    if raw_ready:
        raw, _ = publish_sealed_json(corpus / "offset-060/atoms-prefix-0792.json", {
            "execution_preflight_sha256": "raw-execution", "batch_validation_shas": ["fixture"] * 792})
        publish_sealed_json(raw_campaign / "completed-offset-060.json", {**binding, "raw_completion_sha256": raw.sha256})
    calls = []
    def command(module, *args):
        ...
        if module == "tools.audit_spine_source_admission_v3":
        ...
        elif module == "tools.admit_spine_corpus_v6":
            publish_sealed_json(corpus / "offset-060/source-bound-atoms-prefix-0792.json", {"fixture": True})
        elif module == "tools.verify_spine_admission_method_v10":
            publish_sealed_json(corpus / "offset-060/conditional-method-v10-prefix-0792.json", {
        ...
        elif module == "tools.compile_spine_semantic_index":
            ...
            publish_sealed_json(paths["evaluation"] / "preflight.json", {"shard_offset": 60,
...
def test_complete_memory_prepares_fifty_requests_after_all_compilers(tmp_path, monkeypatch, invalid):
    ...
    worker.run(job, 60, True)
    modules = [module for module, _ in calls]
    assert modules[-4:] == ["tools.compile_spine_semantic_index", "tools.compile_spine_user_addresses",
        "tools.compile_spine_facet_addresses", "tools.evaluate_spine_semantic_seeds"]
    assert modules.index("tools.verify_spine_admission_method_v10") < modules.index("tools.compile_spine_leaf_projection")
    if invalid:
    ...
    prepared = read_sealed_json(campaign / "prepared/offset-060.json")
...
def test_schema_failure_stops_before_qwen_compaction(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="schema failures"):
        worker.run(job, 60, True)
    assert [module for module, _ in calls] == ["tools.audit_spine_source_admission_v3"]
    assert read_sealed_json(job / "failure.json").payload["phase"] == "audit"
    assert not (campaign / "prepared/offset-060.json").exists()
...
def test_failed_child_preserves_reservation_and_stops_following_work(tmp_path, monkeypatch):
    ...
    assert not (campaign / "prepared/offset-060.json").exists()
    assert not any(module == "tools.compile_spine_semantic_index" for module, _ in calls)
