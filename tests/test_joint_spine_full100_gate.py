...
from tools.report_joint_spine_full100 import full100_gate
...
def population():
    ...
    rows = [{"ordinal": i, "question_id": str(i), "arm": arm, "correct": i < correct,
            ...
            for arm, correct in (("summary_hybrid", 94), ("hybrid_qwen", 95)) for i in range(100)]
...
def test_accuracy_and_latency_must_pass_on_one_complete_method():
    ...
    assert not result["summary_hybrid"]["joint_gate_passed"]
    assert result["hybrid_qwen"]["joint_gate_passed"]
    for row in rows:
        if row["arm"] == "hybrid_qwen" and row["ordinal"] >= 90:
            row["memory"]["ttft_s"] = 9
    result = full100_gate(rows, questions)
    assert result["hybrid_qwen"]["accuracy_passed"]
    assert not result["hybrid_qwen"]["joint_gate_passed"]
    assert result["hybrid_qwen"]["latency_ratios"]["matched_api"]["ttft_s"]["p95_s"] == 1.8
...
def test_fast_matched_prompt_does_not_hide_slow_short_chat_comparison():
    ...
    result = full100_gate(rows, questions)["hybrid_qwen"]
