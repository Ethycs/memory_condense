...
def _load(fixture: tuple[Path, Path, SourceValidationPlan, tuple[RawStressShard, ...], dict[str, Any]]):
    path, lock, plan, shards, _payload = fixture
    return load_mem0_comparison_policy(
        path,
        source_plan=plan,
...
def test_policy_builds_exact_retrieval_and_scoring_authorizations(policy_fixture):
    policy = _load(policy_fixture)
    shard = policy_fixture[3][0]

    retrieval = policy.retrieval_authorization(shard)
    assert retrieval.sample_offset == 0
    assert retrieval.authorized_add_operations == 3
    assert retrieval.authorized_extraction_calls == 3
    assert retrieval.authorized_search_operations == 10
    assert retrieval.mem0_provider_retries == 0
    scoring = policy.scoring_authorization(
        shard, retrieval_artifact_sha256=_digest("retrieval")
    )
    assert scoring.authorized_responder_calls == 10
    assert scoring.authorized_judge_calls == 10
    assert scoring.max_prompt_tokens == 8_000
    assert scoring.responder_max_output_tokens == 256
    assert scoring.provider_retries == 0
    assert scoring.mem0_policy_sha256 == policy.sha256
    assert scoring.scoring_policy_sha256 == policy.sha256

    _validate_retrieval_authorization(
        shard,
        retrieval,
        computed_mem0_environment_lock_sha256=(
            policy.environment_lock_sha256
        ),
    )
    _validate_scoring_authorization(
        shard,
        ...
        computed_root_environment_lock_sha256=(
            policy_fixture[2].environment_lock_sha256
        ),
    )
