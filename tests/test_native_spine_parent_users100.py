...
@pytest.mark.parametrize('defect', [None, 'base_route', 'prior_raw', 'audited_layout'])
def test_parent_variant_requires_preserved_baseline_and_audited_packet(tmp_path, defect):
    _, evidence, _ = candidate(tmp_path)
    original = SimpleNamespace(payload=deepcopy(evidence.payload))
    checked = SimpleNamespace(payload={k: deepcopy(evidence.payload[k]['parent_context'])
                                      for k in ('hydration', 'routing', 'rendered')})
    if defect is None:
        evaluation.validate_preservation(evidence, original, checked)
        return
    if defect == 'base_route':
        original.payload['routing']['parent_context']['context_atomic_ids'] = ['foreign']
    elif defect == 'prior_raw':
        original.payload['hydration']['parent_context']['sections'][0]['evidence'][0]['text'] += ' changed'
    else:
        checked.payload['rendered']['text'] += ' changed'
