...
@pytest.mark.parametrize('defect', [None, 'hydration', 'routing', 'rendered'])
def test_layout_comparison_rejects_any_evidence_change(tmp_path, defect):
    _, evidence, _ = candidate(tmp_path)
    original = SimpleNamespace(payload=deepcopy(evidence.payload))
    admitted = SimpleNamespace(payload={k: deepcopy(evidence.payload[k]['parent_context'])
                                       for k in ('hydration', 'routing', 'rendered')})
    if defect is None:
        evaluation.validate_layout_packet(evidence, original, admitted)
    else:
        if defect == 'rendered':
            admitted.payload['rendered']['text'] += ' injected'
        else:
            original.payload[defect]['parent_context']['receipt_sha256'] = 'changed'
        with pytest.raises(ValueError, match='changed retrieval'):
            evaluation.validate_layout_packet(evidence, original, admitted)
