...
from tools import run_reduced_specialist_judge_v3 as judge
...
def _answer_payload(rows: tuple[judge.AnswerSeamRow, ...]) -> dict[str, Any]:
    return {
        ...
        "construction_artifact_sha256": _sha("construction"),
        ...
        "preflight_artifact_sha256": _sha("answer preflight"),
        "question_count": judge.QUESTION_COUNT,
        "required_authorized_provider_calls": judge.QUESTION_COUNT,
...
def _preflight(
    ...
    run = _artifact("specialist v3 answer", _answer_payload(source))
...
def test_answer_seam_requires_scoped_v3_and_exact_byte_identical_replay(
    tmp_path: Path,
) -> None:
    ...
    unscoped = _answer_payload(source)
    unscoped.pop("scoped_completion_format")
    bad_run, _ = publish_sealed_json(tmp_path / "unscoped-run.json", unscoped)
    bad_replay, _ = publish_sealed_json(tmp_path / "unscoped-replay.json", unscoped)
    with pytest.raises(
        judge.ReducedSpecialistJudgeV3Error,
        match="v3 answer envelope changed",
    ):
...
def test_preflight_verifies_v3_replay_before_gold_and_seals_ten_prompts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source, gold = _source_and_gold()
    run = _artifact("verified specialist v3 answer", _answer_payload(source))
    replay = SealedArtifact(Path("answer-replay.json"), run.sha256, run.payload)
    calls: list[str] = []
...
def test_provider_requires_exact_ten_calls_before_environment_access(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_materialize_and_replay_are_checkpoint_only_with_v3_names(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert read_sealed_json(tmp_path / judge.JUDGE_NAME).payload["format"] == (
        judge.JUDGE_FORMAT
    )
    assert read_sealed_json(tmp_path / judge.SCORE_NAME).payload["format"] == (
        judge.SCORE_FORMAT
    )
...
def test_provider_parser_exposes_no_gold_or_answer_authority() -> None:
    ...
    assert "v3" in judge.PREFLIGHT_NAME
    assert "v3" in judge.JUDGE_NAME
