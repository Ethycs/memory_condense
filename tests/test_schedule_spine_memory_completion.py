...
from tools import schedule_spine_memory_completion as scheduler
...
def test_wait_never_releases_on_unfinished_payload(tmp_path, monkeypatch):
    ...
    marker, _ = publish_sealed_json(raw / "completed-offset-060.json", {"offset": 60})
    ...
    assert scheduler.wait_for_raw(60, {}, release, float("inf")).sha256 == marker.sha256
    assert waited == [10]
...
def test_wait_stops_for_failure_terminal_process_or_deadline(tmp_path, monkeypatch, condition, exception):
    ...
    with pytest.raises(exception):
        scheduler.wait_for_raw(60, {}, release, -1 if condition == "expired" else float("inf"))


def test_corrupt_completed_namespace_is_not_treated_as_pending(tmp_path, monkeypatch):
    _, raw, _, release = fixture(tmp_path, monkeypatch)
    artifact, _ = publish_sealed_json(raw / "completed-offset-060.json", {"offset": 60})
    artifact.path.write_bytes(b"corrupt")
    with pytest.raises(SealedArtifactError):
        scheduler.wait_for_raw(60, {}, release, float("inf"))
...
@pytest.mark.parametrize("fail_offset", [None, 70])
def test_serial_workers_prepare_full100_only_after_every_completion(tmp_path, monkeypatch, fail_offset):
    ...
    def command(module, *args):
        ...
        if module == "tools.finish_spine_memory_namespace":
            ...
            assert module == "tools.run_spine_semantic_seed_full100_v2"
    ...
    if fail_offset is None:
        ...
        assert len(result.payload["completed_namespaces"]) == 4
        ...
        assert [r["offset"] for r in failure.payload["completed_namespaces"]] == [60]
        assert failure.payload["offset"] == 70 and failure.payload["automatic_retry_performed"] is False
