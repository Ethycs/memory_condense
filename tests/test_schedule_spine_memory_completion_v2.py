...
from tools import schedule_spine_memory_completion_v2 as scheduler
...
def test_wait_never_releases_on_unfinished_payload(tmp_path, monkeypatch):
    ...
    marker, _ = publish_sealed_json(raw / "completed-offset-070.json", {"offset": 70})
    ...
    assert scheduler.wait_for_raw(70, {}, release, float("inf")).sha256 == marker.sha256
    assert waited == [10]
...
def test_wait_stops_for_failure_terminal_process_or_deadline(tmp_path, monkeypatch, condition, exception):
    ...
    with pytest.raises(exception):
        scheduler.wait_for_raw(70, {}, release, -1 if condition == "expired" else float("inf"))


def test_corrupt_completed_namespace_is_not_treated_as_pending(tmp_path, monkeypatch):
    _, raw, _, release = fixture(tmp_path, monkeypatch)
    artifact, _ = publish_sealed_json(raw / "completed-offset-070.json", {"offset": 70})
    artifact.path.write_bytes(b"corrupt")
    with pytest.raises(SealedArtifactError):
        scheduler.wait_for_raw(70, {}, release, float("inf"))
...
@pytest.mark.parametrize("fail_offset", [None, 80])
def test_serial_workers_prepare_full100_only_after_every_completion(tmp_path, monkeypatch, fail_offset):
    ...
    def command(module, *args):
        ...
        if module == "tools.finish_spine_memory_namespace_v2":
    ...
    if fail_offset is None:
        ...
        assert len(result.payload["completed_namespaces"]) == 3
        ...
        assert [r["offset"] for r in failure.payload["completed_namespaces"]] == [70]
        assert failure.payload["offset"] == 80 and failure.payload["automatic_retry_performed"] is False
