...
from tools import admit_spine_corpus_v5 as admission
from tools import repair_spine_summary_budget_v4 as repair
from tools import verify_spine_admission_method_v6 as verification
from tools import verify_spine_admission_method_v7 as verification_v7
...
@pytest.fixture(autouse=True, params=(verification, verification_v7))
def method_version(request, monkeypatch):
    monkeypatch.setattr(sys.modules[__name__], "verification", request.param)
...
def test_duplicate_support_quote_and_all_oversized_summaries_replay_without_raw_qwen_input(tmp_path, monkeypatch):
    ...
    def record(text, **kwargs):
        ...
        if "atoms" in body:
            body["atoms"][0]["support"] = ["quoted text", "another quote"]
            text = json.dumps(body).replace('"support": ["quoted text", "another quote"]',
                '"support": [""quoted text"", ""another quote""]', 1)
    ...
    assert result.payload["extended_support_delimiter_rule_needed"]
    ...
    policy = read_sealed_json(root / "offset-000/source-binding-policy-v8-prefix-0001.json").payload
    legacy_verifier = verification.previous.previous if verification is verification_v7 else verification.previous
    legacy = {**policy, "format": "memory-condense-spine-source-binding-policy-v7",
        "syntax_repair": legacy_verifier.SYNTAX_RULE,
        "implementation": verification.base.base.legacy.hashes(legacy_verifier.ADMISSION_IMPLEMENTATION)}
