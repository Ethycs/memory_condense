...
def admit(root, offset, limit, summary_repair_root=None):
    if summary_repair_root is None:
        from tools.admit_spine_corpus import admit as legacy_admit
        return legacy_admit(root, offset, limit)
    ...
    if summary_repair_root is not None:
        from tools.recover_spine_summary_budget import run
    ...
    policy, _ = publish_sealed_json(destination / f"source-binding-policy-{version}-prefix-{limit:04d}.json", {
        ...
        "summary_compaction_recovery": "preserve validated original slots; recover failed jobs singly with at most two calls at 48 then 24 words",
