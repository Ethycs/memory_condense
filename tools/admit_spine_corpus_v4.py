...
def admit(root, offset, limit, summary_repair_root=None):
    ...
    if summary_repair_root is not None:
        from tools.repair_spine_summary_budget_v3 import run
    ...
    policy, _ = publish_sealed_json(destination / f"source-binding-policy-{version}-prefix-{limit:04d}.json", {
        ...
        "syntax_repair": "missing or misplaced support-string terminators, or duplicated support-array opening quote; preserve original response and every summary byte",
        ...
        "implementation": {name: hashlib.sha256(Path(name).read_bytes()).hexdigest() for name in (
            "tools/admit_spine_corpus_v4.py", "src/memory_condense/search/spine_source_admission.py",
            "src/memory_condense/search/spine_quote_json_repair.py", "src/memory_condense/search/spine_quote_json_repair_v2.py")}})
