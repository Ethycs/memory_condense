...
def admit(root, offset, limit, summary_repair_root=None):
    ...
    if summary_repair_root is not None:
        repair_format = read_sealed_json(summary_repair_root / "preflight.json").payload["format"]
        if repair_format == "memory-condense-spine-summary-budget-recovery-v2":
            from tools.recover_spine_summary_budget_v2 import run
        elif repair_format == "memory-condense-spine-summary-budget-repair-v4":
            from tools.repair_spine_summary_budget_v4 import run
        else:
            raise ValueError("embedded-support-quote admission requires current bounded summary compaction")
    ...
    policy, _ = publish_sealed_json(destination / f"source-binding-policy-{version}-prefix-{limit:04d}.json", {
        ...
        "syntax_repair": "support delimiter repair and escaping inside curly-quoted support only; preserve original response and every summary byte",
        ...
        "implementation": {name: hashlib.sha256(Path(name).read_bytes()).hexdigest() for name in (
            "tools/admit_spine_corpus_v7.py", "src/memory_condense/search/spine_source_admission.py",
