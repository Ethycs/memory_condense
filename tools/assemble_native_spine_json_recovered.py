"""Admit native bodies with distinct transport and malformed-JSON recovery records."""
...
from tools import assemble_native_spine_recovered as previous
from tools import repair_native_json_batches as json_repair
from tools.matched_eval.artifacts import publish_sealed_json, read_sealed_json
from tools.run_hot_reduced30_answer_judge import _phase_lock

base = direct.base
READY = {"accepted", "repaired", "recovered", "json_repaired"}
FORMAT = "native-spine-json-recovered-body-assembly-v1"


def implementation():
    return {**previous.implementation(), **json_repair.implementation(),
            "tools/assemble_native_spine_json_recovered.py": base.digest(__file__)}


class JsonRecoveredSummaryBodies(base.AdmittedSummaryBodies):
...
verified_recoveries = previous.verified_recoveries


def verified_json_repairs(preflight, roots):
    admitted, receipts = {}, []
    for root in map(Path, roots):
        result = json_repair.execute(root, False)
        plan = read_sealed_json(root / "preflight.json")
        if result.payload["source_preflight_sha256"] != preflight.sha256:
            raise ValueError("JSON repair belongs to another source compilation")
        ...
        for binding in result.payload["admitted_original_batches"]:
            ...
            p = row.payload
            ordinal = p["ordinal"]
            if (row.sha256 != binding["sha256"] or p["json_repair_preflight_sha256"] != plan.sha256
                    ...
                    or p["unchanged_valid_atom_indices"] != []
                    or p["replaced_original_atom_indices"] != list(range(p["original_atom_count"]))):
                raise ValueError("JSON repair admission binding changed")
            if ordinal in admitted:
                raise ValueError("duplicate JSON recovery; select one explicit lineage")
            admitted[ordinal] = row
    return admitted, receipts


def verify_json_row(row, repair):
    original = read_sealed_json(row["original_validation_path"])
    p = repair.payload
    if (row["status"] != "json_repaired" or row["original_status"] != "invalid_summary"
            or row["validation_sha256"] != repair.sha256
            or row["original_validation_sha256"] != original.sha256
            or original.sha256 != p["source_validation_sha256"]
            or original.payload["status"] != "invalid_summary"
            or original.payload["request_sha256"] != p["source_request_sha256"]
            or row["request_sha256"] != p["source_request_sha256"]
            or row["ordinal"] != p["ordinal"]):
        raise ValueError("JSON repair lost its unchanged original failure record")


def body_groups(input_root, preflight, bindings, rows, repairs, recovered, json_repairs):
    ...
    for binding, row in zip(bindings, rows, strict=True):
        ...
        if ready:
            ...
            if row["status"] == "json_repaired":
                verify_json_row(row, json_repairs[row["ordinal"]])
...
def assemble(input_root, output_root, *, legacy_roots=(), direct_roots=(), recovery_roots=(),
             recovery_section_roots=(), json_repair_roots=(), allow_partial=False):
    input_root, output_root = Path(input_root), Path(output_root)
    with _phase_lock(output_root, "native-json-recovered-body-assembly"):
        ...
        json_repairs, json_receipts = verified_json_repairs(preflight, json_repair_roots)
        if json_repairs.keys() & (repairs.keys() | recovered.keys()):
            raise ValueError("JSON recovery overlaps another repair lineage")
        ...
        if saved_path.exists():
            ...
            if (saved["compiler_preflight_sha256"] != preflight.sha256 or saved["model"] != base.MODEL
                    ...
                    or saved["json_repair_results"] != json_receipts
            ...
            for ordinal, (row, binding) in enumerate(zip(saved["rows"], bindings, strict=True)):
                ...
                if row["status"] == "json_repaired":
                    verify_json_row(row, json_repairs[ordinal])
        else:
            ...
            saved.update(producer_implementation=implementation(), recovery_results=recovery_receipts,
                         json_repair_results=json_receipts)
            ...
            for ordinal, result in json_repairs.items():
                row = saved["rows"][ordinal]
                if (row["status"] != "invalid_summary"
                        or row["validation_sha256"] != result.payload["source_validation_sha256"]):
                    raise ValueError("JSON recovery cannot replace this original invalid response")
                row.update(original_status=row["status"], original_validation_path=row["validation_path"],
                           original_validation_sha256=row["validation_sha256"], status="json_repaired",
                           validation_path=str(result.path.resolve()), validation_sha256=result.sha256)
                verify_json_row(row, result)
        ...
        if (output_root/"summary-bodies.json").exists():
            with closing(JsonRecoveredSummaryBodies(output_root)) as store:
        ...
        with closing(sqlite3.connect(raw_path.as_uri()+"?mode=ro", uri=True)) as raw, closing(sqlite3.connect(partial)) as db:
            ...
            for sha, atoms, count, missing in body_groups(input_root, preflight, bindings, saved["rows"], repairs, recovered, json_repairs):
                seen_bodies += 1
                if missing:
                    continue
        ...
        return publish_sealed_json(output_root/"summary-bodies.json", {
            ...
            "json_repaired_batches": len(json_repairs),
