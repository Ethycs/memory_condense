"""Check append-only lexical routing on all 100 questions without answer calls.
...
from memory_condense.application import native_spine_additive_lexical as application
from memory_condense.search import native_spine_additive_lexical as routing
...
def run(root, policy_path):
    ...
    with closing(evaluation.frozen.EmbeddingService(device='cuda', batch_size=8)) as encoder:
        with application.AdditiveLexicalMemoryCondenser(evaluation.APPLICATION / 'application', embedder=encoder,
            ...
            for case in cases:
                ...
                preserved = new_route.get('context_route', new_route)
    ...
    report, _ = publish_sealed_json(root / 'report.json', {
        ...
        'packets_with_addition': sum('context_route' in bound(entry['packet']).payload['routing'] for entry in packets),
