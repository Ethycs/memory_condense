"""Compare summary-term coverage against frozen complete-memory evidence.
...
def audit(root, output):
    ...
    rows = []
    try:
        coverage = SpineTermCoverage(memory.semantic.hierarchy, memory.source_spine)
        for call in p["calls"]:
            if call["arm"] != "base":
                continue
            q = call["question"]
    ...
    artifact, _ = publish_sealed_json(output / "audit.json", {
        ...
        "input_policy": "stored summary terms only; exact raw hydration after selection",
        ...
        "implementation": {name: hashlib.sha256(Path(name).read_bytes()).hexdigest() for name in (
            "tools/audit_spine_term_coverage.py", "src/memory_condense/search/spine_term_coverage.py")}})
