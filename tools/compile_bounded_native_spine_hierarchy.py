...
from tools import compile_bounded_native_spine_exchanges as reused_exchanges
from tools import compile_recovered_native_spine_hierarchy as previous
from tools import compile_expanding_native_spine_hierarchy as legacy
from tools import prepare_bounded_native_spine_attention as attention_admission
...
FORMAT = "native-spine-bounded-parent-producer-v1"


def implementation():
    return {**legacy.implementation(), **bounded.implementation(), **attention_admission.implementation(),
            "tools/compile_bounded_native_spine_hierarchy.py": digest(__file__)}
...
def reusable_parents(roots, backend, sources_sha256, attention_method_sha256, *, chain=()):
    ...
    for source in roots:
        ...
        if p.get('producer_format') != FORMAT:
            cached, legacy_receipts = legacy.reusable_parents([source], backend,
                sources_sha256, attention_method_sha256, chain=chain)
            merge_values(values, cached)
            receipts.extend(legacy_receipts)
            continue
        ...
        ancestors = {}
        if p.get("producer_format") in (FORMAT, previous.FORMAT):
            expected = implementation() if p["producer_format"] == FORMAT else previous.implementation()
            if p["producer_implementation"] != expected:
        ...
        journal = bounded.BoundedJournal(source, plan, backend, 0)
...
def execute(root, exchange_root, attention_root, backend, budget=0, *, reuse_roots=None):
    ...
    with _phase_lock(root, "native-expanded-parent-compilation"):
        ...
        preflight, _ = publish_sealed_json(root/"preflight.json", {
            ...
            "bounded_recovery_variants": list(bounded.VARIANTS),
