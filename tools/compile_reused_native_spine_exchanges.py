"""Expand native exchanges using explicitly authenticated earlier neutral merges."""
import argparse
...
FORMAT = "native-spine-reused-exchanges-v1"


def implementation():
    return {**original.implementation(), "tools/compile_reused_native_spine_exchanges.py": digest(__file__)}
...
def _execute(root, backend, budget=0, *, reuse_roots=None, chain=()):
...
def execute(root, backend, budget=0, *, reuse_roots=None):
    return _execute(root, backend, budget, reuse_roots=reuse_roots)[0]
