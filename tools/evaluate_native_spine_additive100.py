"""Evaluate append-only lexical routing on 100 questions from persisted memory.

Reuse one application ingestion, the unchanged v7 reader and original grading.
The retrieval limits and reader remain fixed; one summary match may append
...
from memory_condense.search import native_spine_additive_lexical as additive_routing
from tools import assess_native_spine_additive_lexical as assessment
...
from tools import evaluate_native_spine_application100 as previous
...
PACKET_BASELINE = Path('eval_results/native-spine-app-direct8-20260915-r1')
ASSESSMENT = Path('eval_results/native-spine-additive-lexical-20260915-r1')
...
def validate_preservation(evidence, original, checked):
    ...
    if (route.get('context_route', route) != old['routing']['parent_context']
...
def validate_plan(plan):
    ...
    checked_packets = validate_assessment(bound(p['packet_assessment']), comparison, policy)
...
def run(root, policy_path, reader_path, enable=False):
    ...
    with ExitStack() as stack:
        ...
        plan, _ = publish_sealed_json(root / 'preflight.json', {'implementation': implementation(),
            ...
            'packet_assessment': binding(checked_report),
