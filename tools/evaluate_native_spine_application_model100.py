"""Compare answer models on identical packets from one persisted application memory.

Questions, retrieval, reader instructions, raw evidence and grading stay fixed.
The selected gateway model is bound in every request and response journal.
"""
import argparse
...
MODEL_INVENTORY = Path('eval_results/native-spine-model-inventory-20260915-r1/models.json')
ANSWER_MODELS = frozenset({'codex_sdk/gpt-5.6-sol', 'codex_sdk/gpt-5.6-terra',
...
validate_pair = previous.validate_pair


def validate_model(model, inventory):
    if (model not in ANSWER_MODELS or model not in inventory.payload['model_ids']
            or inventory.payload['gateway'] != frozen.GATEWAY):
        raise ValueError('requires an admitted raw-answer model on the authorized gateway')
    return model
...
def validate_plan(plan):
    ...
    if (p['implementation'] != implementation() or p['history_count'] != 1
            ...
            or p['new_history_compilations'] != 0 or p['model_only_comparison'] is not True
    ...
    validate_model(p['model'], bound(p['model_inventory']))
    ...
    if (comparison.payload['model'] == p['model']
        ...
        raise ValueError('model comparison must preserve retrieval, reader, questions and ingested memory')
    ...
    for ordinal, case in enumerate(cases):
        ...
        if any(c['messages'] != old['messages'] for c, old in zip(
                calls[ordinal * 2:ordinal * 2 + 2], comparison.payload['calls'][ordinal * 2:ordinal * 2 + 2], strict=True)):
            raise ValueError('model comparison changed a baseline prompt')
    return questions, scope


def recorded(root, plan):
    results = []
    for call in plan.payload['calls']:
        ...
        if not prefix.with_suffix('.response.json').exists():
            if prefix.with_suffix('.reserved').exists() or prefix.with_suffix('.request.json').exists():
                raise ValueError('unacknowledged answer stream; no implicit retry')
            continue
        ...
        validate_response(call, request, response, evidence, plan)
        results.append((call, response))
    if [c['call_index'] for c, _ in results] != list(range(len(results))):
        raise ValueError('answer stream journal contains a gap')
    return results


def validate_response(call, request, response, evidence, plan):
    r, m = response.payload, response.payload['measurement']
    if (request.payload != {'preflight_sha256': plan.sha256, 'call': call}
            or r['request_sha256'] != request.sha256 or r['messages'] != call['messages']
            or m['messages_sha256'] != call['messages_sha256'] or m['model'] != plan.payload['model']
            or m['max_tokens'] != 256 or m['prediction_sha256'] != quote_sha256(m['prediction'])
            or evidence.sha256 != call['evidence_sha256']
            or any(r[k] != evidence.payload[k].get(call['arm']) for k in ('hydration','routing','rendered'))):
        raise ValueError('answer stream lost its selected model, prompt or exact evidence binding')


def seal_answers(root, plan):
    observations = recorded(root, plan)
...
def run(root, policy_path, reader_path, model, enable=False):
    ...
    validate_model(model, inventory)
    ...
    if (comparison.payload['reader_policy'] != binding(reader_policy)
            ...
            or comparison.payload['model'] == model):
        raise ValueError('model comparison requires unchanged retrieval/reader and a different model')
    ...
    with ExitStack() as stack:
        ...
        for case in cases:
            ...
            if any(new != old.payload[k]['parent_context'] for k, new in (
                    ('messages', messages), ('hydration', hydration), ('routing', routing), ('rendered', rendered))):
                raise ValueError('fresh model-comparison retrieval changed baseline evidence or prompt')
        ...
        plan, _ = publish_sealed_json(root / 'preflight.json', {'implementation': implementation(),
            ...
            'model_only_comparison': True, 'model_inventory': binding(inventory),
        ...
        print({'preflight_sha256': plan.sha256, 'history_count': 1, 'question_count': 100,
               'body_tokens': admission['body_tokens'], 'answer_streams': 200, 'model': model,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', choices=sorted(ANSWER_MODELS))
    parser.add_argument('--enable-provider', action='store_true')
    args = parser.parse_args()
    if args.phase == 'run':
        run(args.root, args.policy, args.reader_policy, args.model, args.enable_provider)
