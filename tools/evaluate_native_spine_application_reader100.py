"""Compare sealed reader instructions on 100 questions from persisted application memory.
...
from tools import evaluate_native_spine_threaded100 as previous
...
PACKET_BASELINE = Path('eval_results/native-spine-threaded100-app-20260915-r1')
...
def validate_reader_policy(policy):
    if (set(policy) != {'format', 'name', 'baseline_system_sha256', 'system_prompt'}
            or policy['format'] != 'native-spine-reader-instructions-v1'
            or type(policy['name']) is not str or not policy['name'].strip()
            or policy['baseline_system_sha256'] != quote_sha256(reader.SPINE_READER_SYSTEM_PROMPT_V6)
            or type(policy['system_prompt']) is not str or not policy['system_prompt'].strip()
            or count_tokens(policy['system_prompt']) > 2048):
        raise ValueError('requires bounded, sealed reader instructions relative to v6')
...
def validate_plan(plan):
    ...
    if (comparison.payload['context_policy'] != p['context_policy']
        ...
        raise ValueError('reader comparison changed its evidence or presentation')
    ...
    for ordinal, case in enumerate(cases):
        ...
        expected_messages = reader.apply_reader(comparison.payload['calls'][ordinal * 2]['messages'],
                                                reader_policy.payload['system_prompt'])
        if (tuple(c['arm'] for c in group) != (ARMS if ordinal % 2 == 0 else ARMS[::-1])
                ...
                or any(c['messages'] != expected_messages for c in group)
                or any(c['messages_sha256'] != identity_sha256(c['messages']) for c in group)):
            raise ValueError('reader schedule, raw context, question, or matched prompt changed')
...
def run(root, policy_path, reader_path, enable=False):
    ...
    if comparison.payload['context_policy'] != binding(policy):
        raise ValueError('reader comparison must reuse the sealed baseline retrieval policy')
    ...
    with ExitStack() as stack:
        ...
        for case in cases:
            ...
            if (hydration != old.payload['hydration']['parent_context']
                    ...
                    or rendered != old.payload['rendered']['parent_context']
                    or messages[1:] != old.payload['messages']['parent_context'][1:]
                    or messages != reader.apply_reader(old.payload['messages']['parent_context'],
                                                      reader_policy.payload['system_prompt'])):
                raise ValueError('reader comparison changed selected evidence, rendering or question')
        ...
        print({'preflight_sha256': plan.sha256, 'history_count': 1, 'question_count': 100,
               ...
               'context_policy': policy.payload, 'reader_only': reader_policy.payload['name']}, flush=True)
