"""Evaluate a sealed context policy on 100 questions and one cached live memory.
...
def build(memory, question, policy):
    return context_policy.build(memory, question, policy)
...
def audit(root):
    ...
    with closing(sqlite3.connect(bank_path.as_uri() + '?mode=ro', uri=True)) as raw:
        ...
        for call, response in observations:
            if call['arm'] != 'parent_context':
                continue
            messages, count = context_policy.verify_packet(call['question'], response.payload['hydration'],
                response.payload['routing'], sessions, load_body, policy.payload)
...
def run(root, policy_path, enable=False):
    ...
    with closing(frozen.EmbeddingService(device='cuda', batch_size=8)) as encoder:
        ...
        for case in cases:
            ...
            messages, hydration, routing = build(memory, q, policy.payload)
        ...
        print({'preflight_sha256': plan.sha256, 'history_count': 1, 'question_count': 100,
               ...
               'context_policy': policy.payload}, flush=True)
        with closing(frozen._completion_client('LITELLM_KEY', frozen.GATEWAY)) as client:
            for call in calls:
                ...
                prepared = {}
                def prompt():
                    m, h, r = build(memory, call['question'], policy.payload) if call['arm'] == 'parent_context' else (call['messages'], None, None)
