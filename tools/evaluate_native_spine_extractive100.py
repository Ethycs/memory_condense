"""Evaluate an extractive reader on identical persisted user-spine packets.

All 100 original questions, references, retrieval and grading remain fixed.
This exposed development comparison changes only sealed system instructions.
"""
import argparse
...
from memory_condense.eval import spine_reader_policy_v8 as reader
...
def validate_reader_policy(policy):
    prompt = previous.validate_reader_policy(policy)
    if policy['name'] != 'user-extraction-v8' or prompt != reader.SPINE_READER_SYSTEM_PROMPT_V8:
        raise ValueError('reader comparison requires the fixed extractive v8 policy')
    return prompt
...
def validate_preservation(evidence, original):
    p, old = evidence.payload, original.payload
    if (any(p[k] != old[k] for k in ('routing', 'hydration', 'rendered'))
            or set(p['messages']) != set(old['messages'])
            or any(p['messages'][a][1:] != old['messages'][a][1:] for a in ARMS)):
        raise ValueError('reader comparison changed retrieval, raw evidence, layout or question')
...
def validate_plan(plan):
    ...
    if (comparison.payload['context_policy'] != p['context_policy']
            ...
            or p['retrieval_and_grading_unchanged'] is not True
        ...
        raise ValueError('reader comparison changed retrieval, questions or ingested memory')
...
def judge(root, enable=False):
    ...
    report, _ = publish_sealed_json(root / 'joint-report.json', {
        ...
        'retrieval_and_grading_unchanged': True,
...
def run(root, policy_path, reader_path, enable=False):
    ...
    if comparison.payload['context_policy'] != binding(policy):
        raise ValueError('reader comparison must reuse the sealed retrieval policy')
