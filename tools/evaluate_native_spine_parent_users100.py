"""Evaluate append-only parent-user summary routing on 100 questions from persisted memory.

Reuse one application ingestion, the unchanged v7 reader and original grading.
The retrieval limits and reader remain fixed; bounded parent-user matches may append
after the complete baseline route. No question-specific inputs enter routing. This exposed development set does not establish generalization.
"""
import argparse
...
from tools import evaluate_native_spine_user_layout100 as previous
...
PACKET_BASELINE = Path('eval_results/native-spine-app-userlayout100-20260915-r1')
ASSESSMENT = Path('eval_results/native-spine-parent-users-coverage-20260915-r1')
...
def validate_assessment(report, comparison, policy):
    p = report.payload
    plan = bound(p['preflight'])
    packets = bound(p['packets'])
    if (p['prior_routes_and_hydrated_evidence_preserved'] is not True
            or p['verified_memory_packets'] != 100 or p['question_count'] != 100
            or p['history_count'] != 1 or p['lost_all_support_ordinals']
            or p['new_answer_calls'] != 0 or p['answer_accuracy_measured'] is not False
            or p['routing_strategy'] != additive_routing.FORMAT
            or plan.payload['baseline'] != binding(comparison)
            or plan.payload['policy'] != binding(policy)
            or packets.payload['preflight'] != binding(plan)
            or packets.payload['packet_count'] != 100
            or any(digest(path) != sha for path, sha in plan.payload['implementation'].items())):
        raise ValueError('additive route requires complete unchanged provider-free preservation audit')
    rows = packets.payload['packets']
    if [r['ordinal'] for r in rows] != list(range(100)):
        raise ValueError('additive packet population changed')
    parent_report = bound(plan.payload['parent_compilation'])
    compilation = bound(parent_report.payload['preflight'])
    if (digest(parent_report.payload['parent_file']['path']) != parent_report.payload['parent_file']['sha256']
            or parent_report.payload['original_application_files_unchanged'] is not True
            or parent_report.payload['new_ingestions'] != 0
            or compilation.payload['native_snapshot'] != comparison.payload['admission']
            or compilation.payload['question_inputs'] is not False
            or compilation.payload['gold_inputs'] is not False
            or any(digest(path) != sha for path, sha in compilation.payload['implementation'].items())):
        raise ValueError('compiled parent vectors or base application changed')
    return rows


def validate_preservation(evidence, original, checked):
    p, old, admitted = evidence.payload, original.payload, checked.payload
    route = p['routing']['parent_context']
    additive_routing.route_from_payload(route)
    sections = p['hydration']['parent_context']['sections']
    old_sections = old['hydration']['parent_context']['sections']
    if (route.get('parent_base', route) != old['routing']['parent_context']
            or sections[:len(old_sections)] != old_sections
            or any(p[k]['parent_context'] != admitted[k] for k in ('routing', 'hydration', 'rendered'))):
        raise ValueError('additive packet changed prior evidence or its audited packet')
...
def validate_plan(plan):
    ...
    checked_report = bound(p['packet_assessment'])
    checked_packets = validate_assessment(checked_report, comparison, policy)
    parent_compilation = bound(bound(checked_report.payload['preflight']).payload['parent_compilation'])
    if p['parent_admission'] != parent_compilation.payload['parent_snapshot']:
        raise ValueError('parent application admission changed')
    if (comparison.payload['context_policy'] != p['context_policy']
        ...
        raise ValueError('retrieval comparison changed the reader, questions or ingested memory')
    ...
    for ordinal, case in enumerate(cases):
        ...
        checked = bound(checked_packets[ordinal]['packet'])
        if checked.payload['question'] != frozen.question(case):
            raise ValueError('audited question changed')
        validate_preservation(evidence, old, checked)
    return questions, scope


seal_answers = previous.seal_answers
...
def run(root, policy_path, reader_path, enable=False):
    ...
    previous.validate_plan(comparison)
    checked_report = read_sealed_json(ASSESSMENT / 'report.json')
    validate_assessment(checked_report, comparison, policy)
    if (comparison.payload['reader_policy'] != binding(reader_policy)
            or comparison.payload['context_policy'] != binding(policy)):
        raise ValueError('retrieval comparison must reuse the sealed baseline reader policy')
    ...
    with ExitStack() as stack:
        ...
        print({'preflight_sha256': plan.sha256, 'history_count': 1, 'question_count': 100,
               ...
               'context_policy': policy.payload, 'reader': reader_policy.payload['name']}, flush=True)
