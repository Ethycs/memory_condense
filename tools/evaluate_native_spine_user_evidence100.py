"""Evaluate a whole-section assistant-context ablation on the same Sol reader.

Preserve the v7 reader, routing, hydration, questions, grading and lifecycle.
Only whole assistant-only sections are omitted from the served presentation.
...
from tools import native_spine_user_evidence_presentation as presentation
from tools import evaluate_native_spine_parent_sol100 as previous
...
PACKET_BASELINE = Path('eval_results/native-spine-app-parent-sol100-20260915-r1')
APPLICATION = Path('eval_results/native-spine-application-lifecycle-20260915-r1')
PROJECTION_CHECK = Path('eval_results/native-spine-user-evidence-admission-20260915-r1/report.json')
...
def implementation():
    return {**app_lifecycle.implementation(), str(baseline.__file__): digest(baseline.__file__),
            'tools/assess_native_spine_user_evidence.py': digest('tools/assess_native_spine_user_evidence.py'),
            ...
            str(presentation.renderer.layout.__file__): digest(presentation.renderer.layout.__file__),
...
def validate_preservation(evidence, original):
    candidate, baseline_packet = evidence.payload, original.payload
    if any(candidate[k] != baseline_packet[k] for k in ('hydration', 'routing')):
        raise ValueError('projection comparison changed its baseline hydration or routing')
    hydrated = baseline_packet['hydration']['parent_context']
    has_user = any(e['span']['role'] == 'user' for s in hydrated['sections'] for e in s['evidence'])
    excluded = [s for s in hydrated['sections'] if has_user and s['evidence']
                and all(e['span']['role'] == 'assistant' for e in s['evidence'])]
    excluded_ids = [s['section']['section_id'] for s in excluded]
    retained = {e['span']['receipt_sha256']: e['text'] for s in hydrated['sections']
                if s['section']['section_id'] not in excluded_ids for e in s['evidence']}
    rendered = candidate['rendered']['parent_context']
    placements = rendered['placements']
    if (rendered['format'] != presentation.renderer.FORMAT
            or rendered['source_hydration_sha256'] != hydrated['receipt_sha256']
            or rendered['omitted_section_ids'] != excluded_ids
            or len(placements) != len(retained)
            or {p['span_sha256'] for p in placements} != set(retained)
            or any(rendered['text'][p['start_char']:p['end_char']] != retained[p['span_sha256']]
                   for p in placements)):
        raise ValueError('projection changed its required exact whole-section evidence')
...
def validate_plan(plan):
    ...
    if (p['implementation'] != implementation() or p['history_count'] != 1
            ...
            or p['context_projection_only_comparison'] is not True
    ...
    checked_projection = bound(p['projection_assessment'])
    if (checked_projection.payload['baseline'] != binding(comparison)
            or checked_projection.payload['implementation'] != implementation()
            or checked_projection.payload['question_count'] != 100
            or [r['ordinal'] for r in checked_projection.payload['packets']] != list(range(100))):
        raise ValueError('projection admission changed its baseline, implementation or population')
    if (comparison.payload['model'] != p['model']
            ...
            or p['hydration_routing_and_questions_identical_to_baseline'] is not True
            or p['live_retrieval_inside_timer'] is not True
            or p['unchanged_ingested_memory'] is not True):
        raise ValueError('projection comparison changed model, reader, retrieval, questions or ingested memory')
    ...
    for ordinal, case in enumerate(cases):
        ...
        checked = bound(checked_projection.payload['packets'][ordinal]['packet'])
        if evidence.payload != checked.payload:
            raise ValueError('live projection differs from independently audited raw packet')
...
def judge(root, enable=False):
    ...
    report, _ = publish_sealed_json(root / 'joint-report.json', {
        ...
        'model': plan.payload['model'], 'context_projection_only_comparison': True,
        'reader_and_grading_unchanged': True,
        'hydration_routing_and_questions_identical_to_baseline': True,
        ...
        'projection_assessment': plan.payload['projection_assessment'],
...
def audit(root):
    ...
    packets = spans = served_spans = omitted_sections = 0
    with closing(sqlite3.connect(bank_path.as_uri() + '?mode=ro', uri=True)) as raw:
        ...
        for call, response in observations:
            ...
            packets += 1
            spans += count
            served_spans += len(response.payload['rendered']['placements'])
            omitted_sections += len(response.payload['rendered']['omitted_section_ids'])
    result, _ = publish_sealed_json(root / 'raw-audit.json', {'joint_report': binding(report),
        'verified_memory_packets': packets, 'verified_raw_spans': spans,
        'verified_served_raw_spans': served_spans, 'omitted_assistant_only_sections': omitted_sections,
        'new_model_calls': 0,
        'history_count': 1, 'question_count': 100})
    print({'raw_audit_sha256': result.sha256, 'verified_memory_packets': packets, 'verified_raw_spans': spans,
           'verified_served_raw_spans': served_spans, 'omitted_assistant_only_sections': omitted_sections}, flush=True)
    return result
...
def run(root, policy_path, reader_path, enable=False):
    ...
    projection_check = read_sealed_json(PROJECTION_CHECK)
    if (comparison.payload['context_policy'] != binding(policy)
            or comparison.payload['reader_policy'] != binding(reader_policy)):
        raise ValueError('projection comparison must reuse the sealed retrieval and reader policies')
    ...
    with ExitStack() as stack:
        ...
        plan, _ = publish_sealed_json(root / 'preflight.json', {'implementation': implementation(),
            ...
            'model_inventory': binding(inventory), 'context_projection_only_comparison': True,
            ...
            'projection_assessment': binding(projection_check),
            'reader_and_grading_unchanged': True,
            'hydration_routing_and_questions_identical_to_baseline': True,
