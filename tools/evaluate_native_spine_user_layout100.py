"""Evaluate user-spine-first presentation on 100 questions from persisted memory.

Keep retrieval, raw evidence, the v7 reader, model and original grading fixed.
Change only the layout: user statements precede other turns within each source. This exposed development set does not establish generalization.
"""
import argparse
...
from memory_condense.application import native_spine_additive_lexical as additive_application
from tools import assess_native_spine_user_layout as layout_assessment
MemoryCondenser = additive_application.AdditiveLexicalMemoryCondenser
...
PACKET_BASELINE = Path('eval_results/native-spine-app-additive100-20260915-r2')
ASSESSMENT = Path('eval_results/native-spine-user-layout-preflight-20260915-r1')
...
def implementation():
    return {**layout_assessment.implementation(), **app_lifecycle.implementation(), str(baseline.__file__): digest(baseline.__file__),
...
def validate_layout_assessment(report, comparison, policy):
    p = report.payload
    plan = bound(p['preflight'])
    if (plan.payload['implementation'] != layout_assessment.implementation()
            or plan.payload['baseline'] != binding(comparison)
            or plan.payload['policy'] != binding(policy)
            or plan.payload['presentation'] != presentation.renderer.FORMAT
            or p['hydration_and_routing_unchanged'] is not True
            or p['verified_memory_packets'] != 100 or p['history_count'] != 1
            or p['question_count'] != 100 or p['new_answer_calls'] != 0
            or p['references_opened'] is not False or p['answer_accuracy_measured'] is not False
            or [r['ordinal'] for r in p['packets']] != list(range(100))):
        raise ValueError('user-spine layout requires a complete unchanged raw audit')
    return p['packets']


def validate_layout_packet(evidence, original, checked):
    candidate, old, admitted = evidence.payload, original.payload, checked.payload
    if (any(candidate[k]['parent_context'] != old[k]['parent_context'] for k in ('hydration', 'routing'))
            or any(candidate[k]['parent_context'] != admitted[k] for k in ('hydration', 'routing', 'rendered'))):
        raise ValueError('layout experiment changed retrieval or its audited raw presentation')
...
def validate_plan(plan):
    ...
    if (p['implementation'] != implementation() or p['history_count'] != 1
            ...
            or p['presentation'] != presentation.renderer.FORMAT
            or p['retrieval_and_hydration_unchanged'] is not True):
    ...
    checked = validate_layout_assessment(bound(p['layout_assessment']), comparison, policy)
    ...
    for ordinal, case in enumerate(cases):
        ...
        original = read_sealed_json(comparison.path.parent / 'evidence' / f'{ordinal:03d}.json')
        packet = bound(checked[ordinal]['packet'])
        if packet.payload['question'] != frozen.question(case):
            raise ValueError('layout assessment question changed')
        validate_layout_packet(evidence, original, packet)
...
def judge(root, enable=False):
    ...
    report, _ = publish_sealed_json(root / 'joint-report.json', {
        ...
        'presentation': presentation.renderer.FORMAT, 'layout_assessment': plan.payload['layout_assessment'],
...
def run(root, policy_path, reader_path, enable=False):
    ...
    validate_layout_assessment(checked_report, comparison, policy)
    ...
    with ExitStack() as stack:
        ...
        plan, _ = publish_sealed_json(root / 'preflight.json', {'implementation': implementation(),
            ...
            'lifecycle': dict(LIFECYCLE), 'presentation': presentation.renderer.FORMAT,
            'layout_assessment': binding(checked_report), 'retrieval_and_hydration_unchanged': True,
