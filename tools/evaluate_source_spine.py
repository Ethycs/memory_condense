"""Compare exchange hydration with bounded exact user-spine source expansion.
...
MEMORY_ARMS = ("spine_union", "source_spine")
ARMS = ("short_api", "spine_union", "spine_union_api", "source_spine", "source_spine_api")
IMPLEMENTATION = ("tools/evaluate_source_spine.py",
    "src/memory_condense/search/source_spine_hydration.py", "src/memory_condense/eval/_binary_judge_protocol.py",
    "src/memory_condense/eval/fast_completion_runtime.py", "tools/evaluate_user_spine_real_pilot.py", "tools/compile_spine_semantic_index.py",
    "src/memory_condense/search/summary_semantic_index.py", "src/memory_condense/search/spine_union_routing.py",
    "src/memory_condense/search/summary_query_view.py", "src/memory_condense/search/summary_time_prior.py",
...
def prepare(root, index_root, addresses_root, atoms_path):
    ...
    memory = ResidentMemory(index_root, manifest.sha256, addresses_root, atoms_path)
    calls = []
    try:
        for i, question in enumerate(questions):
            messages = {"short_api": answer_messages(question)}
            for arm in MEMORY_ARMS:
                hydrated = memory.retrieve(question["retrieval_query"], arm, question["prompt_question"])
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-source-spine-eval-v1", "index_root": str(index_root.resolve()),
        ...
        "calendar_frontier": 32, "calendar_max_sections_per_source": 1, "selected_section_cap": 14, "lexical_reserve": 2,
...
class ResidentMemory:
    def __init__(self, index_root, expected_sha, addresses_root, atoms_path, expected_addresses_sha=None, expected_atoms_sha=None):
        ...
        addresses = read_sealed_json(addresses_root / "addresses.json")
        matrix_path = addresses_root / "spine-vectors.npy"
        if (addresses.payload["base_index_sha256"] != manifest.sha256 or
                (expected_addresses_sha is not None and addresses.sha256 != expected_addresses_sha) or
                addresses.payload["matrix_sha256"] != hashlib.sha256(matrix_path.read_bytes()).hexdigest()):
            raise ValueError("user-spine address binding changed")
        user = UserSpineAddressIndex(self.semantic.hierarchy,
            np.load(index_root / "summary-vectors.npy", allow_pickle=False),
            np.load(matrix_path, allow_pickle=False), embedding_identity=self.semantic.embedding_identity)
        if user.receipt_sha256 != addresses.payload["address_index_sha256"]:
            raise ValueError("user-spine address metadata changed")
        self.addresses_sha256 = addresses.sha256
        self.router = SpineUnionRouter(self.semantic, user)
        atoms = read_sealed_json(atoms_path)
        if not atoms.payload["complete_namespace"] or (expected_atoms_sha is not None and atoms.sha256 != expected_atoms_sha):
            raise ValueError("complete source-atom binding changed")
        self.atoms_sha256 = atoms.sha256
        self.source_spine = SourceSpineHydrationIndex(self.semantic.hierarchy,
            tuple(SectionSummary.from_dict(a) for a in atoms.payload["atoms"]))
        self.retrieve("Initialize memory search.", "spine_union", "Initialize memory search.")
    ...
    def retrieve(self, query, arm, dated_question):
        ...
        plan, _ = self.router.route_vectors(query, dated_question, vectors[0], embedding_identity=identity,
                                            content_vector=vectors[1] if len(vectors) == 2 else None)
        if arm == "source_spine":
            plan, _ = self.source_spine.expand(plan)
        return hydrate_section_plan(plan, load_turn=self.turns.get,
            max_context_tokens=3072 if arm == "source_spine" else 4096, max_raw_spans=128)
...
def run(root, max_calls):
    ...
    memory = ResidentMemory(Path(preflight.payload["index_root"]), preflight.payload["index_manifest_sha256"],
        ...
        preflight.payload["addresses_sha256"], preflight.payload["atoms_sha256"])
        ...
        for call in pending[:max_calls]:
            ...
            def prompt():
                ...
                if call["arm"] in MEMORY_ARMS:
                    hydrated = memory.retrieve(q["retrieval_query"], call["arm"], q["prompt_question"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--addresses-root", type=Path)
    parser.add_argument("--atoms", type=Path)
    parser.add_argument("--max-calls", type=int)
    parser.add_argument("--enable-provider", action="store_true")
    args = parser.parse_args()
    if args.phase == "prepare":
        prepare(args.output_root, args.index_root, args.addresses_root, args.atoms)
    elif args.phase == "run":
