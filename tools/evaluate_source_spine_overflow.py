"""Compare protected source-spine hydration with additional routed user turns.
...
from tools.evaluate_source_spine import ResidentMemory as BaseResidentMemory
...
MEMORY_ARMS = ("source_spine", "source_spine_overflow")
ARMS = ("short_api", "source_spine", "source_spine_api", "source_spine_overflow", "source_spine_overflow_api")
IMPLEMENTATION = ("tools/evaluate_source_spine_overflow.py", "tools/evaluate_source_spine.py",
...
def prepare(root, index_root, addresses_root, atoms_path):
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-source-spine-overflow-eval-v1", "index_root": str(index_root.resolve()),
...
class ResidentMemory(BaseResidentMemory):
    def __init__(self, *args):
        super().__init__(*args)
        self.overflow = SourceSpineOverflow(self.source_spine)

    def retrieve(self, query, arm, dated_question):
        # The base constructor performs its query-independent warmup through
        # the original union route before the overflow metadata is installed.
        if arm in {"spine_union", "source_spine"}:
            return super().retrieve(query, arm, dated_question)
        if arm != "source_spine_overflow":
        ...
        plan, _ = self.overflow.expand(selected)
