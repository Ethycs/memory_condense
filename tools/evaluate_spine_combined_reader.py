"""Separate reader qualification from whole-summary source coverage on complete memories.
...
from memory_condense.eval.spine_reader_policy_v3 import SPINE_READER_SYSTEM_PROMPT_V3 as SPINE_READER_SYSTEM_PROMPT
...
MEMORY_ARMS = ("base", "reader", "combined_reader")
ARMS = ("base_short_api", "reader_short_api", *(a for arm in MEMORY_ARMS for a in (arm, arm + "_api")))
ROUTES = {"base": "source_spine_diverse", "reader": "source_spine_diverse",
          "combined_reader": "source_spine_combined"}
SHORT_CONTROLS = {"base": "base_short_api", "reader": "reader_short_api", "combined_reader": "reader_short_api"}
COMBINED_COVERAGE_POLICY = {"summary_frontier": 32, "additional_sources": 8,
    "channel": "whole section summary", "raw_reads_during_routing": 0,
    "prior_user_prefix_preserved": True, "prior_context_may_be_displaced": True,
    "order": "source-diverse and scoped-term users, whole-summary source users, prior context",
    "additional_query_embeddings": 0, "query_time_qwen_calls": 0}
IMPLEMENTATION = tuple(dict.fromkeys((*SOURCE_IMPLEMENTATION,
    "tools/evaluate_spine_combined_reader.py", "tools/spine_combined_memory.py",
...
def reader_policies():
    return {arm: quote_sha256(QA_SYSTEM_PROMPT if arm == "base" else SPINE_READER_SYSTEM_PROMPT)
...
def prepare(root, index_root, addresses_root, atoms_path, facets_root):
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-spine-combined-reader-eval-v1", "index_root": str(index_root.resolve()),
        ...
        "combined_coverage": COMBINED_COVERAGE_POLICY, "routes": ROUTES, "short_controls": SHORT_CONTROLS,
        ...
        "baseline": "each reader policy has one short API control; every memory arm has its own identical-evidence API control",
        "reader_policies": reader_policies(),
        "identical_evidence_arms": ["base", "reader"],
        "identical_reader_arms": ["reader", "combined_reader"],
...
def load_preflight(root):
    ...
    p = artifact.payload
    if (p["format"] != "memory-condense-joint-spine-combined-reader-eval-v1" or
            p["reader_policies"] != reader_policies() or p["routes"] != ROUTES or
            p["short_controls"] != SHORT_CONTROLS or p["combined_coverage"] != COMBINED_COVERAGE_POLICY or
            p["source_coverage"] != SOURCE_COVERAGE_POLICY or p["term_coverage"] != TERM_COVERAGE_POLICY or
            p["identical_evidence_arms"] != ["base", "reader"] or
            p["identical_reader_arms"] != ["reader", "combined_reader"]):
