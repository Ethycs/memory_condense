"""Compare implicit and explicit recommendation personalization on identical evidence.
...
from memory_condense.eval.spine_reader_policy import SPINE_READER_SYSTEM_PROMPT as QA_SYSTEM_PROMPT
from memory_condense.eval.spine_reader_policy_v2 import SPINE_READER_SYSTEM_PROMPT_V2 as SPINE_READER_SYSTEM_PROMPT
from tools.evaluate_source_spine import ResidentMemory
...
MEMORY_ARMS = ("base", "reader")
ARMS = ("base_short_api", "reader_short_api", "base", "base_api", "reader", "reader_api")
IMPLEMENTATION = ("tools/evaluate_spine_reader_v2.py", "src/memory_condense/eval/spine_reader_policy_v2.py", "tools/evaluate_source_spine.py",
...
def call_arm_order(ordinal):
    ...
    shorts = ("base_short_api", "reader_short_api") if ordinal % 2 == 0 else ("reader_short_api", "base_short_api")
    groups = [shorts, *pairs]
...
def prepare(root, index_root, addresses_root, atoms_path):
    ...
    try:
        for i, question in enumerate(questions):
            messages = {arm + "_short_api": answer_messages(question, policy=arm) for arm in MEMORY_ARMS}
            for arm in MEMORY_ARMS:
                hydrated = memory.retrieve(question["retrieval_query"], "source_spine", question["prompt_question"])
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-spine-reader-eval-v2", "index_root": str(index_root.resolve()),
        ...
        "baseline": "each reader policy has its own short API and identical-evidence API controls",
        "reader_policies": {"base": quote_sha256(QA_SYSTEM_PROMPT), "reader": quote_sha256(SPINE_READER_SYSTEM_PROMPT)},
        "evidence_identical_between_reader_policies": True,
...
def validate_matched_calls(calls):
    ...
    for group in groups.values():
        ...
        if any(call["question"] != group["base_short_api"]["question"] for call in group.values()):
        ...
        for arm in MEMORY_ARMS:
            ...
            if group[arm]["messages"][0] != group[arm + "_short_api"]["messages"][0]:
                raise ValueError("short API control uses another reader policy")
        if group["base"]["messages"][1:] != group["reader"]["messages"][1:]:
            raise ValueError("reader comparison changed the evidence or question")


def answer_messages(question, hydrated=None, *, policy="base"):
    if policy not in MEMORY_ARMS:
        raise ValueError("unknown reader policy")
    system = QA_SYSTEM_PROMPT if policy == "base" else SPINE_READER_SYSTEM_PROMPT
        ...
        for call in pending[:max_calls]:
            ...
            def prompt():
                ...
                if call["arm"] in MEMORY_ARMS:
                    hydrated = memory.retrieve(q["retrieval_query"], "source_spine", q["prompt_question"])
...
def judge(root, enable):
    ...
    ratios = {arm: {baseline: {metric: {stat: timing[arm][metric][stat] / timing[baseline][metric][stat]
                    ...
                    for baseline in (arm + "_short_api", arm + "_api")} for arm in MEMORY_ARMS}
