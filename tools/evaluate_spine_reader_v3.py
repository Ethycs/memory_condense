"""Compare concise qualified answering with the v2 reader on identical passage-routed evidence.
...
from tools.spine_facet_memory import ResidentMemory
from tools.evaluate_source_spine_facets import IMPLEMENTATION as FACET_IMPLEMENTATION
...
IMPLEMENTATION = tuple(dict.fromkeys((*FACET_IMPLEMENTATION,
    "tools/evaluate_spine_reader_v3.py", "src/memory_condense/eval/spine_reader_policy_v3.py")))
...
def prepare(root, index_root, addresses_root, atoms_path, facets_root):
    ...
    try:
        for i, question in enumerate(questions):
            ...
            for arm in MEMORY_ARMS:
                hydrated = memory.retrieve(question["retrieval_query"], "source_spine_facets", question["prompt_question"])
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-spine-reader-eval-v3", "index_root": str(index_root.resolve()),
...
def load_preflight(root):
    ...
    p = artifact.payload
    expected_policies = {"base": quote_sha256(QA_SYSTEM_PROMPT), "reader": quote_sha256(SPINE_READER_SYSTEM_PROMPT)}
    if (p["format"] != "memory-condense-joint-spine-reader-eval-v3" or
            p["reader_policies"] != expected_policies or p["evidence_identical_between_reader_policies"] is not True):
        raise ValueError("reader policy comparison changed")
        ...
        for call in pending[:max_calls]:
            ...
            def prompt():
                ...
                if call["arm"] in MEMORY_ARMS:
                    hydrated = memory.retrieve(q["retrieval_query"], "source_spine_facets", q["prompt_question"])
