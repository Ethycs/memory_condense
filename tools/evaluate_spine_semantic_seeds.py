"""Compare semantic seed priority with the unchanged source-diverse control.
...
MEMORY_ARMS = ("base", "semantic_seeds")
ARMS = ("short_api", *(a for arm in MEMORY_ARMS for a in (arm, arm + "_api")))
ROUTES = {"base": "source_spine_diverse", "semantic_seeds": "source_spine_semantic_seeds"}
SHORT_CONTROLS = {arm: "short_api" for arm in MEMORY_ARMS}
SEED_POLICY = {"baseline_sections": 6, "control_lexical_reserve": 2, "candidate_lexical_reserve": 0,
    "channel": "whole section summary", "raw_reads_during_routing": 0,
    "calendar_and_user_supplements_unchanged": True,
    "source_diversity_and_scoped_terms_unchanged": True,
    "prior_user_evidence_preservation_required": False,
    "additional_query_embeddings": 0, "query_time_qwen_calls": 0}
IMPLEMENTATION = tuple(dict.fromkeys((*SOURCE_IMPLEMENTATION,
    "tools/evaluate_spine_semantic_seeds.py", "tools/spine_semantic_seed_memory.py",
...
def prepare(root, index_root, addresses_root, atoms_path, facets_root):
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-spine-semantic-seeds-eval-v1", "index_root": str(index_root.resolve()),
        ...
        "seed_policy": SEED_POLICY, "routes": ROUTES, "short_controls": SHORT_CONTROLS,
...
def load_preflight(root):
    ...
    p = artifact.payload
    if (p["format"] != "memory-condense-joint-spine-semantic-seeds-eval-v1" or
            p["reader_policies"] != reader_policies() or p["routes"] != ROUTES or
            p["short_controls"] != SHORT_CONTROLS or p["seed_policy"] != SEED_POLICY or
