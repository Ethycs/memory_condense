"""Compare passage, scoped-term and source-diverse routing on complete memories.
...
MEMORY_ARMS = ("source_spine_facets", "source_spine_term_coverage", "source_spine_diverse")
ARMS = ("short_api", *(arm for memory in MEMORY_ARMS for arm in (memory, memory + "_api")))
TERM_COVERAGE_POLICY = {"source_scope": "previously summary-selected conversations",
    "rare_query_terms": 2, "leaves_per_term": 2, "maximum_document_frequency": "max(1, floor(leaf_count / 100))",
    "rarity_population": "complete memory", "raw_reads_during_routing": 0,
    "prior_user_prefix_preserved": True, "prior_context_may_be_displaced": True}
SOURCE_COVERAGE_POLICY = {"summary_frontier": 32, "sources_per_channel": 8,
    "channels": ["user_summary", "user_summary_passage"], "raw_reads_during_routing": 0,
    "prior_user_prefix_preserved": True, "prior_context_may_be_displaced": True,
    "order": "prior users, source-diverse users, scoped term coverage, exact hydration"}
IMPLEMENTATION = ("tools/evaluate_spine_source_coverage.py", "tools/spine_source_memory.py",
    "src/memory_condense/search/spine_source_coverage.py", "tools/spine_term_memory.py",
    "src/memory_condense/search/spine_term_coverage.py", "src/memory_condense/search/spine_term_coverage_v2.py",
    "tools/evaluate_source_spine_facets.py", "tools/evaluate_source_spine_overflow.py", "tools/spine_facet_memory.py",
    "src/memory_condense/search/spine_summary_facets.py", "src/memory_condense/search/source_spine_supplement.py", "tools/evaluate_source_spine.py",
    "src/memory_condense/search/source_spine_overflow.py", "src/memory_condense/eval/spine_reader_policy_v2.py",
    "src/memory_condense/eval/spine_reader_policy.py",
...
def prepare(root, index_root, addresses_root, atoms_path, facets_root):
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-spine-source-coverage-eval-v1", "index_root": str(index_root.resolve()),
        "index_manifest_sha256": manifest.sha256, "addresses_root": str(addresses_root.resolve()),
        "addresses_sha256": memory.addresses_sha256, "facets_root": str(facets_root.resolve()),
        "facets_sha256": memory.facets_sha256, "atoms_path": str(atoms_path.resolve()),
...
def load_preflight(root):
    ...
    if p["term_coverage"] != TERM_COVERAGE_POLICY:
        raise ValueError("term coverage policy changed")
    if p["source_coverage"] != SOURCE_COVERAGE_POLICY:
        raise ValueError("source coverage policy changed")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--facets-root", type=Path)
    parser.add_argument("--max-calls", type=int)
    parser.add_argument("--enable-provider", action="store_true")
    args = parser.parse_args()
    if args.phase == "prepare":
        prepare(args.output_root, args.index_root, args.addresses_root, args.atoms, args.facets_root)
    elif args.phase == "run":
