"""Versioned comparison of summary hybrid and additive user-spine routing.
...
MEMORY_ARMS = ("summary_hybrid", "spine_union")
ARMS = ("short_api", "summary_hybrid", "summary_hybrid_api", "spine_union", "spine_union_api")
IMPLEMENTATION = ("tools/evaluate_spine_union.py", "tools/compile_spine_semantic_index.py",
...
def prepare(root, index_root, addresses_root):
    ...
    memory = ResidentMemory(index_root, manifest.sha256, addresses_root)
    calls = []
    try:
    ...
    artifact, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-joint-spine-union-eval-v1", "index_root": str(index_root.resolve()),
        "index_manifest_sha256": manifest.sha256, "addresses_root": str(addresses_root.resolve()),
        "addresses_sha256": memory.addresses_sha256, "probes_sha256": probes.sha256, "shard_offset": p["shard_offset"],
...
class ResidentMemory:
    def __init__(self, index_root, expected_sha, addresses_root, expected_addresses_sha=None):
    ...
    def retrieve(self, query, arm, dated_question):
        if arm == "summary_hybrid":
            plan = self.semantic.route(query, encoder=self.encoder, max_sections=6, lexical_reserve=2)
        elif arm == "spine_union":
...
def run(root, max_calls):
    ...
    memory = ResidentMemory(Path(preflight.payload["index_root"]), preflight.payload["index_manifest_sha256"],
        Path(preflight.payload["addresses_root"]), preflight.payload["addresses_sha256"])
...
if __name__ == "__main__":
    ...
    if args.phase == "prepare":
        prepare(args.output_root, args.index_root, args.addresses_root)
    elif args.phase == "run":
