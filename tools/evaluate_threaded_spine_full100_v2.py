...
IMPLEMENTATION = tuple(dict.fromkeys((*previous.IMPLEMENTATION,
    'tools/evaluate_threaded_spine_full100.py', 'tools/evaluate_threaded_spine_full100_v2.py',
...
def prepare(root, source_root):
    """Reuse unexecuted authenticated prompts; only repair the missing judge import."""
    from tools import evaluate_threaded_spine_full100 as predecessor
    original = predecessor.load_preflight(source_root)
    if root.resolve() == source_root.resolve():
        raise ValueError('the repaired evaluator requires a separate output root')
    if (source_root / 'execution.reserved').exists() or (source_root / 'journal').exists():
        raise ValueError('prepared prompt reuse requires an unexecuted predecessor')
    if original.payload['policy'] != POLICY:
        raise ValueError('the repaired evaluator must preserve the complete policy')
    for ordinal in range(100):
        source = read_sealed_json(source_root / 'evidence' / f'{ordinal:03}.json')
        copied, _ = publish_sealed_json(root / 'evidence' / f'{ordinal:03}.json', source.payload)
        if copied.sha256 != source.sha256:
            raise ValueError('prepared evidence changed while copying')
    preflight, _ = publish_sealed_json(root / 'preflight.json', {
        **original.payload, 'implementation': implementation(),
        'format': 'memory-condense-threaded-spine-joint-full100-v2',
        'prepared_predecessor_root': str(source_root.resolve()),
        'prepared_predecessor_sha256': original.sha256,
        'repair': 'import the existing binary judge prompt builder; prompts and policy unchanged'})
    load_preflight(root)
    print({'preflight_sha256': preflight.sha256, 'answer_calls': 400,
           'prepared_prompts_reused': True, 'new_provider_calls': 0}, flush=True)
...
def load_preflight(root):
    ...
    p = artifact.payload
    from tools import evaluate_threaded_spine_full100 as predecessor
    prepared = predecessor.load_preflight(Path(p['prepared_predecessor_root']))
    if (prepared.sha256 != p['prepared_predecessor_sha256'] or
            any(p[key] != prepared.payload[key] for key in
                ('calls', 'namespaces', 'source_root', 'source_preflight_sha256', 'policy',
                 'reader_sha256', 'model', 'gateway', 'gold_loaded'))):
        raise ValueError('repaired runner changed its prepared predecessor')
