...
from tools.spine_transport_lineage_v3 import stage_state


IMPLEMENTATION = ("tools/execute_spine_transport_recovery_v3.py", "tools/spine_transport_lineage_v3.py",
    "tools/spine_session_transport_stage.py", "tools/execute_spine_corpus.py",
...
def execution_payload(state):
    ...
    return {"format": "memory-condense-spine-transport-successor-execution-v3",
        ...
        "requires_admission_verifier": "tools.verify_spine_admission_method_v10",
...
def prepare(root):
    ...
    artifact, _ = publish_sealed_json(root / "execution-raw-v3.json", execution_payload(state))
...
def run(root, readiness_path=None, enable=False):
    ...
    marker = root / "execution-raw-v3.reserved"
    if remaining:
        ...
        publish_sealed_json(root / "readiness" / f"raw-v3-{readiness.sha256}.json", {
