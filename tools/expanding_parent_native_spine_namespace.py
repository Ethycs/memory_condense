...
from tools.compile_expanding_native_spine_hierarchy import FORMAT, implementation
...
from tools.prepare_expanding_native_spine_attention import validate_admission


class ExpandingParentNativeSpineCorpus(ParentBudgetNativeSpineCorpus):
    def __init__(self, source_root, template_store_root, hierarchy_report, *, active_store_root=None):
        ...
        with closing(JsonRecoveredSummaryBodies(template_store_root)) as store:
        ...
        if active_store_root is not None:
            expanded = None
            try:
                expanded = JsonRecoveredSummaryBodies(active_store_root)
