...
from tools import run_spine_semantic_seed_full100_v2 as evaluation_runner
from tools.matched_eval.artifacts import read_sealed_json, publish_sealed_json


CAMPAIGN = Path("eval_results/full1m-spine-semantic-seeds-full100-20260910-r2")
PROTOCOL_SHA = "c8b27afa6589d548c0225afef120ae1a61929822febf08f5fd03508e173da99e"
...
IMPLEMENTATION = ("tools/finish_spine_memory_namespace.py", "tools/audit_spine_source_admission_v3.py",
    "tools/repair_spine_summary_budget_v4.py", "tools/finish_spine_compaction_batches.py",
    "tools/recover_spine_summary_budget_v2.py", "tools/admit_spine_corpus_v6.py",
    "tools/verify_spine_admission_method_v10.py", "tools/compile_spine_leaf_projection.py",
    "tools/compile_spine_semantic_index.py", "tools/compile_spine_user_addresses.py",
...
def inputs(offset):
    if type(offset) is not int or offset not in (60, 70, 80, 90):
        raise ValueError("only the four remaining full100 namespaces may be compiled here")
...
def plan_payload(offset):
    ...
    return {"format": "memory-condense-complete-remaining-spine-memory-v1", "offset": offset,
...
def run(root, offset, enable=False):
    ...
    try:
        ...
        command("tools.audit_spine_source_admission_v3", *common, "--output", paths["audit"] / "audit.json")
        ...
        command("tools.admit_spine_corpus_v6", *common, *repair_args)
        atoms_path = corpus_root / f"offset-{offset:03d}" / f'source-bound-atoms-prefix-{binding["request_count"]:04d}.json'
        command("tools.verify_spine_admission_method_v10", "--atoms", atoms_path, *repair_args)
        verification = read_sealed_json(atoms_path.with_name(atoms_path.name.replace("source-bound-atoms", "conditional-method-v10")))
        if verification.payload["method_sha256"] != protocol.payload["source_admission_method_sha256"]:
            raise ValueError("new memory admission differs from the first six memories")
        ...
        command("tools.compile_spine_semantic_index", "--corpus-root", corpus_root, "--shard-offset", offset,
