...
IMPLEMENTATION = ("tools/finish_spine_memory_namespace_v2.py", "tools/audit_spine_source_admission_v4.py",
...
def inputs(offset):
    if type(offset) is not int or offset not in (70, 80, 90):
        raise ValueError("only the three remaining full100 namespaces may be compiled here")
...
def plan_payload(offset):
    ...
    return {"format": "memory-condense-complete-remaining-spine-memory-v2", "offset": offset,
