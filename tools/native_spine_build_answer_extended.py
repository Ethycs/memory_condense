"""Generation-only replay worker with full tool history and a 72-action ceiling."""
import argparse
...
def answer(root, step):
    ...
    adapter = replay.publish(root / 'extended-tool-context-adapter.json', {
        ...
        'previous_adapter': replay.evaluation.binding(replay.load(root / 'full-tool-context-adapter.json')),
    ...
    messages = [{'role': 'system', 'content': replay.SYSTEM.replace('36 actions', '72 actions')}, {'role': 'user', 'content': content}]
