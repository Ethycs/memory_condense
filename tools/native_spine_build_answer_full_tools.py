"""Generation-only replay worker retaining all tools from the current user turn."""
import argparse
...
def answer(root, step):
    ...
    adapter = replay.publish(root / 'full-tool-context-adapter.json', {
    ...
    ordinal = replay.action_number(folder)
    ...
    messages = [{'role': 'system', 'content': replay.SYSTEM}, {'role': 'user', 'content': content}]
