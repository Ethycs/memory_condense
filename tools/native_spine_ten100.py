"""Ten additional histories, exactly 100 new questions each, using the frozen application.
...
PRIOR_FIVE = Path("eval_results/native-spine-five100-20260916-r1/campaign.json")
...
def implementation():
    return {**current.implementation(), __file__: digest(__file__),
            'tools/complete_native_spine_five100_questions.py': digest('tools/complete_native_spine_five100_questions.py')}
...
def plan(root):
    ...
    if artifact.payload['source_ordinals'] != list(ORDINALS):
        raise ValueError('campaign must remain exactly ten explicitly selected histories')
    return artifact


def history(root, batch):
    if batch not in range(1, 11):
        raise ValueError('batch must be 1 through 10')
    campaign = plan(root)
...
def prepare(root):
    ...
    prior_five = read_sealed_json(PRIOR_FIVE)
    for prior_scope in prior_five.payload['histories']:
        used.update(s['body_sha256'] for s in bound(prior_scope).payload['author_sources'])
    histories = []
    with closing(sqlite3.connect(bank.as_uri() + '?mode=ro', uri=True)) as raw:
        for batch, ordinal in enumerate(ORDINALS, 1):
            ...
            chosen = sorted(eligible.values(), key=lambda s: identity_sha256(
                ['ten100-20260922-v1', ordinal, s['body_sha256']]))[:100]
            ...
            scope = publish(root / f'history-{batch:02d}' / 'scope.json', {
                ...
                'source_selection': 'fixed salted hash; excludes original 100, previous 500, and earlier question bodies in this campaign',
    ...
    artifact = publish(root / 'campaign.json', {'implementation': implementation(),
        'source_ordinals': list(ORDINALS), 'histories': histories, 'history_count': 10,
        'question_count_per_history': 100, 'question_count_total': 1000,
        'prior_five_campaign': binding(prior_five), 'previous_run': binding(previous), 'context_policy': previous.payload['context_policy'],
    ...
    emit(phase='campaign_locked', sha256=artifact.sha256, histories=10, total_questions=1000)
...
def author(root, batch):
    ...
    def one(ordinal, source):
        ...
        validated, corrections = exact_supports(payload, source)
        publish(prefix.with_suffix('.validation.json'), {'response': binding(response), 'quote_corrections': corrections,
            'question_and_answer_unchanged': True, 'candidate_outputs_loaded': False})
        return ordinal, validated, response
