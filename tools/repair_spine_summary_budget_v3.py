...
IMPLEMENTATION = ("tools/repair_spine_summary_budget_v3.py", "tools/repair_spine_summary_budget.py",
...
def prepare(root, corpus_root, audit_path, offset, limit):
    ...
    result, _ = publish_sealed_json(root / "preflight.json", {
        "format": "memory-condense-spine-summary-budget-repair-v3",
...
def run(root, enable=False):
    ...
    if (p["format"] != "memory-condense-spine-summary-budget-repair-v3" or
