...
from tools.verify_spine_admission_method_v11 import load_verified_method as load_method
...
def report(roots, output_root):
    ...
    for root in roots:
        ...
        for judged in joint.payload["rows"]:
            ...
            timing = {}
            for key, method in (("memory", arm), ("matched_api", arm + "_api"), ("short_api", evaluation.short_control(arm))):
    ...
    artifact, _ = publish_sealed_json(output_root / "joint-full100.json", {
        "format": "memory-condense-joint-spine-as-of-full100-v1", "probes_sha256": probes.sha256,
        ...
        "admission_comparison": "replayed conditional source admission v11 including support-only embedded quote escaping and terminal-session raw recovery, preserved summaries, bounded invalid-slot recovery, and compaction-only transport accounting",
