...
from tools import evaluate_spine_namespace as evaluation
...
def report(roots, output_root):
    ...
    common_policy = compilation_policy = None
    for root in roots:
    ...
    artifact, _ = publish_sealed_json(output_root / "joint-full100.json", {
        "format": "memory-condense-joint-spine-full100-v1", "probes_sha256": probes.sha256,
        "namespace_bindings": bindings, "same_implementation_all_namespaces": True,
        "hierarchy_compilation_policy_sha256": compilation_policy, "rows": records, "gates": gates,
