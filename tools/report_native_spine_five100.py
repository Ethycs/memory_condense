"""Aggregate exactly five complete, separately scored 100-question histories."""
import argparse
import math
...
def report(root):
    ...
    if len(timings) != 500 or any(not math.isfinite(v) or v < 0 for v in timings):
        raise ValueError('aggregate requires 500 finite nonnegative timing observations')
    output = {'campaign': evaluation.binding(campaign), 'histories': rows,
        'accuracy': {'correct': correct, 'questions': 500, 'fraction': correct / 500},
        ...
        'combined_latency': {'median_s': statistics.median(timings), 'p95_s': timings[474],
        ...
        'distinct_question_source_bodies': len(question_sources), 'exact_raw_packets_verified': 500,
    ...
    text = ['# Five new 100-question tests', '',
        f'**{correct}/500 ({correct / 5:.1f}%)** under the unchanged original semantic grader.', '',
    ...
    text.extend(['', 'All 500 source-grounded questions were locked before candidate answering. Each history used normal application ingestion and a separate-process reopen. The five histories reused existing Qwen summary/attention caches. All 500 raw packets passed independent reconstruction.', '',
    ...
    evaluation.emit(phase='five100_complete', correct=correct, questions=500, report_sha256=artifact.sha256,
