"""Bind the JSON-recovery compiler to the unchanged complete joint full100 eval."""
...
from tools import run_local_spine_json_recovery as compiler
...
def prepare(root,compiler_root,evaluation_root,source_root,pid,create_time):
    ...
    if (not process.is_running() or process.create_time()!=create_time
        ...
        or 'tools.run_local_spine_json_recovery' not in command or 'compile' not in command
        or '--output-root' not in command
        or Path(command[command.index('--output-root')+1]).resolve()!=compiler_root.resolve()):
        raise ValueError('handoff requires the live bound JSON-recovery compiler process')
    recovery = compiler.validate_preflight(compiler_root)
    inputs = read_sealed_json(compiler_root/'input-snapshots'/'population.json')
    preserved = read_sealed_json(compiler_root/'preserved-completed-memories.json')
    ...
    if (inputs.sha256!=recovery.payload['input_population_sha256']
        or [r['offset'] for r in inputs.payload['records']]!=list(range(0,100,10))
        or first.payload['backend_sha256']!=recovery.payload['backend_sha256']
        or preserved.payload['new_model_calls']!=0 or not preserved.payload['records']
        or any(r['same_summaries_topology_and_spans'] is not True
            or r['original_leaves_unchanged'] is not True for r in preserved.payload['records'])):
        raise ValueError('JSON-recovery release or completed-memory preservation changed')
    preflight,_ = publish_sealed_json(root/'preflight.json',{
        'format':'memory-condense-hierarchy-after-json-recovery-v1','workspace':str(workspace),
        'dependency':{'pid':pid,'create_time':create_time},
        'compiler_root':str(compiler_root.resolve()),'compiler_recovery_sha256':recovery.sha256,
        'preserved_memories_sha256':preserved.sha256,
        'input_population_sha256':inputs.sha256,'parent_method_sha256':first.payload['method_policy_sha256'],
        'compiler_implementation':recovery.payload['implementation'],
...
def validate_preflight(root):
    ...
    if (compiler.validate_preflight(compiler_root).sha256!=p['compiler_recovery_sha256']
        or read_sealed_json(compiler_root/'preserved-completed-memories.json').sha256!=p['preserved_memories_sha256']
