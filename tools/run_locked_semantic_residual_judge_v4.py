#!/usr/bin/env python3
"""Judge replay-authenticated locked semantic-residual V4 answers.

This is a thin identity adapter over the audited full-100 Sol judge lifecycle.
The shared engine authenticates the sealed V4 answer run and byte-identical
replay before opening locked gold, then seals exactly 100 unique judge prompts.
Provider execution remains checkpointed, retry-free, and zero-state.
"""
...
from tools import run_locked_semantic_residual_answer_v4 as answer_v4  # noqa: E402
...
PREFLIGHT_FORMAT = "memory-condense-locked-semantic-residual-sol-judge-preflight-v4"
JUDGE_FORMAT = "memory-condense-locked-semantic-residual-sol-judge-v4"
SCORE_FORMAT = "memory-condense-locked-semantic-residual-sol-score-v4"

ANSWER_RUN_NAME = answer_v4.RUN_NAME
ANSWER_REPLAY_NAME = answer_v4.REPLAY_NAME
PREFLIGHT_NAME = "locked-semantic-residual-sol-judge-preflight-v4.json"
JUDGE_NAME = "locked-semantic-residual-semantic-judge-sol-v4.json"
JUDGE_REPLAY_NAME = "locked-semantic-residual-semantic-judge-sol-replay-v4.json"
SCORE_NAME = "locked-semantic-residual-score-v4.json"
SCORE_REPLAY_NAME = "locked-semantic-residual-score-replay-v4.json"
CHECKPOINT_DIR_NAME = "sol-locked-semantic-residual-judge-calls-v4"

REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_ANSWER_ROOT = answer_v4.DEFAULT_OUTPUT
DEFAULT_JUDGE_ROOT = REPOSITORY_ROOT / (
    "eval_results/matched_eval_100/locked-semantic-residual-sol-judge-v4-r7"
)
...
class LockedSemanticResidualJudgeV4Error(MatchedEvalContractError):
    """Raised when the full-100 semantic-residual V4 judge contract changes."""


def _require(ok: object, message: str) -> None:
    if not ok:
        raise LockedSemanticResidualJudgeV4Error(message)
...
def build_runtime(
    ...
    *,
    ...
    _require(
        ...
        "full-100 semantic-residual V4 judge runtime differs from sealed preflight",
    )
    return FastCompletionRuntime(
        ...
        benchmark_provenance={
            ...
            "arm": "locked_semantic_residual_sol_judge_v4",
...
__all__ = [
    ...
    "JUDGE_NAME",
    "JUDGE_REPLAY_NAME",
    "LockedSemanticResidualJudgeV4Error",
    "PREFLIGHT_FORMAT",
    "PREFLIGHT_NAME",
    "QUESTION_COUNT",
    "SCORE_FORMAT",
