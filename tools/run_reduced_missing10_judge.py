#!/usr/bin/env python3
"""Judge the replay-verified reduced exact-ten answer diagnostic with Sol.

The answer run and its byte-identical replay are completely validated before
this program opens locked benchmark gold.  Provider execution consumes only a
sealed population of ten standard judge prompts; materialization and replay
consume immutable completion journals without provider access.
"""
...
from tools import run_reduced_missing10_answer_diagnostic as answer_cli  # noqa: E402
...
ANSWER_RUN_FORMAT = answer_cli.RUN_FORMAT
PREFLIGHT_FORMAT = "memory-condense-reduced-missing10-sol-judge-preflight-v1"
JUDGE_FORMAT = "memory-condense-reduced-missing10-sol-judge-v1"
SCORE_FORMAT = "memory-condense-reduced-missing10-sol-score-v1"

ANSWER_RUN_NAME = answer_cli.RUN_NAME
ANSWER_REPLAY_NAME = answer_cli.REPLAY_NAME
PREFLIGHT_NAME = "reduced-missing10-judge-preflight-v1.json"
JUDGE_NAME = "reduced-missing10-semantic-judge-sol-v1.json"
JUDGE_REPLAY_NAME = "reduced-missing10-semantic-judge-sol-replay-v1.json"
SCORE_NAME = "reduced-missing10-score-v1.json"
SCORE_REPLAY_NAME = "reduced-missing10-score-replay-v1.json"
CHECKPOINT_DIR_NAME = "sol-reduced-missing10-judge-calls-v1"

DEFAULT_ANSWER_ROOT = answer_cli.DEFAULT_OUTPUT
DEFAULT_JUDGE_ROOT = DEFAULT_ANSWER_ROOT / "sol-judge-v1"
...
class ReducedMissing10JudgeError(MatchedEvalContractError):
    """Raised when the reduced judge boundary is not exact."""


def _require(ok: object, message: str) -> None:
    if not ok:
        raise ReducedMissing10JudgeError(message)
...
def _answer_rows(payload: Mapping[str, Any]) -> tuple[AnswerSeamRow, ...]:
    ...
    _require(
        ...
        "reduced answer judge-source envelope changed",
    )
    result: list[AnswerSeamRow] = []
    ids: list[str] = []
    for expected_ordinal, raw in zip(EXACT_ORDINALS, raw_rows, strict=True):
        _require(type(raw) is dict, "reduced answer judge row changed type")
        ...
        _require(
            ...
            f"reduced answer judge row changed at ordinal {expected_ordinal}",
        )
    ...
    _require(len(set(ids)) == QUESTION_COUNT, "reduced answer question IDs repeat")
    return tuple(result)
...
def load_verified_answer_seam(
    *,
    ...
    """Verify the gold-free answer plane and exact replay before gold access."""
    ...
    _require(
        ...
        "reduced answer run/replay are not byte-identical",
    )
...
def build_preflight_payload(
    *,
    ...
    _require(
        tuple(row.ordinal for row in answer_rows) == EXACT_ORDINALS
        ...
        "reduced judge preflight population/settings changed",
    )
    ...
    for source, gold in zip(answer_rows, gold_rows, strict=True):
        _require(
            ...
            f"reduced judge gold binding changed at ordinal {source.ordinal}",
        )
        messages = tuple(
            dict(row)
...
def validate_preflight_artifact(
    ...
    _require(
        ...
        and type(raw_rows) is list
        and len(raw_rows) == QUESTION_COUNT,
        "sealed reduced judge preflight changed",
    )
    prompts: list[tuple[dict[str, str], ...]] = []
    rows: list[dict[str, Any]] = []
    for expected_ordinal, raw in zip(EXACT_ORDINALS, raw_rows, strict=True):
        _require(type(raw) is dict, "sealed reduced judge prompt row changed type")
        body = dict(raw)
        ...
        _require(
            ...
            f"sealed reduced judge prompt row changed at ordinal {expected_ordinal}",
        )
    ...
    _require(
        ...
        "sealed reduced judge prompt population changed",
    )
...
def build_runtime(
    ...
    *,
    ...
    _require(
        ...
        "reduced judge runtime differs from sealed preflight",
    )
    return FastCompletionRuntime(
        ...
        benchmark_provenance={
            ...
            "arm": "reduced_missing10_sol_judge_v1",
...
def materialization_payloads(
    ...
    _require(
        ...
        "reduced judge materialization requires ten checkpoint hits",
    )
    records = {record.messages_sha256: record for record in batch.unique_records}
    _require(len(records) == QUESTION_COUNT, "reduced judge completion IDs repeat")
    rows: list[dict[str, Any]] = []
    for prompt, completion in zip(
        ...
        _require(
            ...
            "reduced judge checkpoint record changed",
        )
        correct = parse_binary_judge_verdict(completion)
        _require(type(correct) is bool, "reduced judge returned an invalid verdict")
...
def _read_preflight(
    ...
    _require(
        ...
        "reduced judge preflight SHA-256 changed",
    )
...
def run_preflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Deliberately complete before the first locked dataset/split read.
...
def run_provider(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _require(
        ...
        == QUESTION_COUNT,
        "reduced Sol judge requires exact authorization for 10 calls",
    )
    _require(
        ...
        "reduced Sol judge runtime differs from sealed preflight",
    )
    load_dotenv()
    ...
    _require(
        ...
        "reduced Sol journal population changed after authorization",
    )
    return {
...
def run_replay(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _require(
        ...
        "reduced judge materialization differs from checkpoint replay",
    )
    ...
    _require(
        ...
        "reduced judge replay is not byte-identical",
    )
    return {
        "byte_identical": True,
