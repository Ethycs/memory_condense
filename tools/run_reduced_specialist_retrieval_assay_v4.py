#!/usr/bin/env python3
"""Seal the typed-composer-v2 reduced-specialist successor assay.

The frozen v2 and v3 entry points retain their historical composer-v1
semantics.  This entry point gives post-dedup backfill a new protocol identity,
filenames, and output root so successor bytes cannot collide with either
sealed parent experiment.
"""
...
FORMAT = "memory-condense-reduced-specialist-retrieval-assay-v4"
CONSTRUCTION_FORMAT = f"{FORMAT}-construction"
AUDIT_FORMAT = f"{FORMAT}-posthoc-target-audit"
CONSTRUCTION_NAME = "reduced-specialist-construction-v4.json"
AUDIT_NAME = "reduced-specialist-target-audit-v4.json"

REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT_ROOT = REPOSITORY_ROOT / (
    "eval_results/matched_eval_100/reduced-specialist-missing10-v4"
)
...
def run_construct(args: argparse.Namespace) -> dict[str, Any]:
    payload = base.build_construction(
        ...
        typed_composition_mode=base.POST_DEDUP_BACKFILL_COMPOSITION_MODE,
    )
...
def run_audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    base._require(  # noqa: SLF001
        ...
        == require_sha256(
            ...
            "expected specialist v4 construction",
        ),
        "specialist v4 construction artifact changed",
    )
    ...
    base._require(  # noqa: SLF001
        ...
        == base.SUCCESSOR_TYPED_COMPOSITION_FORMAT,
        "specialist v4 typed-composition binding changed",
    )
