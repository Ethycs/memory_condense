"""Run one frozen full100 as-of comparison with isolated answer timing."""
...
from tools import report_joint_spine_as_of_full100 as reporting
...
BULK = Path("eval_results/full100-spine-after-offset060-timeout-20260910-r1")
BULK_SHA = "ed2c488757508298b4f88410713fa0f1d52fea3f334b5009bceb0387ac776033"
IMPLEMENTATION = ("tools/run_spine_as_of_full100.py", "tools/run_spine_reader_after_timeout.py",
    "tools/execute_spine_transport_recovery.py", "tools/probe_spine_gateway_readiness.py")
PROTOCOL_IMPLEMENTATION = (*evaluation.IMPLEMENTATION, "tools/report_joint_spine_as_of_full100.py",
    "tools/prepare_spine_as_of_full100.py",
    "tools/verify_spine_admission_method_v10.py", "tools/verify_spine_admission_method_v11.py",
    "tools/admit_spine_corpus_v7.py", "src/memory_condense/search/spine_quote_json_repair_v4.py", "tools/spine_compaction_transport.py",
    "tools/spine_session_transport_stage.py", "tools/spine_transport_lineage_v3.py",
...
NAMESPACE_FIELDS = {"index_root", "index_manifest_sha256", "shard_offset", "raw_token_proxy", "calls",
    ...
    "facets_root", "facets_sha256", "control_preflight_root", "control_preflight_sha256"}
...
def prepared_roots(root):
    ...
    if (p["required_offsets"] != list(OFFSETS) or p["question_count"] != 100 or
            ...
            p["reader_policies"] != evaluation.reader_policies() or p["as_of_policy"] != evaluation.AS_OF_POLICY or
...
def runner_payload(protocol, bindings):
    return {"format": "memory-condense-as-of-full100-runner-v1", "protocol_sha256": protocol.sha256,
...
def require_bulk_complete():
    ...
    if (preflight.sha256 != BULK_SHA or completion.payload["preflight_sha256"] != preflight.sha256 or
            completion.payload["maximum_new_raw_calls"] != 2925 or
            completion.payload["additional_original_unknown_attempts"] != 6 or
            [(r["offset"], r["request_count"]) for r in completion.payload["completed_namespaces"]] !=
            [(60, 792), (70, 838), (80, 830), (90, 868)]):
        raise ValueError("remaining bulk ingestion and preserved recovery must complete before timed evaluation")
    for row, binding in zip(completion.payload["completed_namespaces"], preflight.payload["bindings"], strict=True):
        if any(row.get(key) != value for key, value in binding.items()):
            raise ValueError("remaining raw completion changed its prepared namespace binding")
        raw = read_sealed_json(Path(binding["corpus_root"]) /
        ...
        if row["offset"] == 60:
            ...
            transport_root = Path(binding["corpus_root"]).parent
            transport = read_sealed_json(transport_root / "execution-complete.json")
            lineage = verify_transport_lineage(Path(binding["corpus_root"]), 60, None)
            if (transport.sha256 != row["transport_execution_complete_sha256"] or
                    transport.payload["transport_lineage"] != lineage or
                    lineage["additional_raw_attempts"] != 6 or
                    lineage["preserved_completed_raw_responses"] != 403):
                raise ValueError("preserved raw recovery accounting changed")
    return completion
