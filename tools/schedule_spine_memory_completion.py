...
IMPLEMENTATION = ("tools/schedule_spine_memory_completion.py", *worker.IMPLEMENTATION,
    "tools/run_spine_semantic_seed_full100_v2.py")
...
def plan_payload(root, release):
    ...
    return {"format": "memory-condense-scheduled-spine-memory-completion-v1",
...
def run(root, enable=False):
    ...
    try:
        for row in plan.payload["workers"]:
            ...
            worker.command("tools.finish_spine_memory_namespace", "run", "--output-root", job,
        ...
        worker.command("tools.run_spine_semantic_seed_full100_v2", "prepare", "--output-root", worker.CAMPAIGN)
