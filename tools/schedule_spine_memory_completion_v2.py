...
IMPLEMENTATION = ("tools/schedule_spine_memory_completion_v2.py", *worker.IMPLEMENTATION,
...
def plan_payload(root, release):
    ...
    return {"format": "memory-condense-scheduled-spine-memory-completion-v2",
...
def run(root, enable=False):
    ...
    try:
        for row in plan.payload["workers"]:
            ...
            worker.command("tools.finish_spine_memory_namespace_v2", "run", "--output-root", job,
