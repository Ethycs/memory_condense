"""Live semantic-seed candidate; existing routing arms keep their original path."""
...
class ResidentMemory(SourceMemory):
    def __init__(self, *args):
        super().__init__(*args)
        self.semantic_seed_router = SemanticSpineSeedRouter(self.semantic, self.router.user_addresses)

    def semantic_seed_plan(self, query, dated_question):
        ...
        selected, seed_audit = self.semantic_seed_router.route_vectors(query, dated_question, vectors[0],
        ...
        return final, {"seeds": seed_audit, "source_coverage": source_audit, "term_coverage": term_audit}

    def retrieve(self, query, arm, dated_question):
        if arm != "source_spine_semantic_seeds":
            return super().retrieve(query, arm, dated_question)
        plan, _ = self.semantic_seed_plan(query, dated_question)
