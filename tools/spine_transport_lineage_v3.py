...
from tools import spine_transport_lineage_v2 as previous
...
from tools.spine_session_transport_stage import validate
...
def stage_state(corpus_root, offset, *, complete=False):
    ...
    plan = validate(root)
    p = plan.payload
    if (s["format"] != "memory-condense-spine-transport-recovery-stage-result-v3" or
...
def verify_transport_lineage(corpus_root, offset, repair_root):
    ...
    if not stage_path.exists() or read_sealed_json(stage_path).payload.get("format") != "memory-condense-spine-transport-recovery-stage-result-v3":
    ...
    from tools.execute_spine_transport_recovery_v3 import execution_payload
    execution = read_sealed_json(stage_path.parent / "execution-raw-v3.json")
    if execution.payload != execution_payload(state):
        raise ValueError("transport executor protocol or call budget changed")
    marker = stage_path.parent / "execution-raw-v3.reserved"
    ...
    starts = list((stage_path.parent / "readiness").glob("raw-v3-*.json"))
    if len(starts) != 1:
