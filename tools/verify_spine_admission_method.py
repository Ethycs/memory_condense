"""Replay source admission and certify its conditional summary-budget rule.
...
No questions, gold answers, raw Qwen inputs, or new provider calls are allowed.
"""
import argparse
...
def conditional_method(policy):
    ...
    return {
        "format": "memory-condense-conditional-source-admission-method-v1",
        ...
        "compaction": {"model": MODEL, "gateway": GATEWAY, "max_output_tokens_per_summary": 128,
            "maximum_jobs_per_batch": 8, "maximum_provider_calls_per_namespace": 1, "retries": 0,
...
def verify(atoms_path, repair_root=None):
    ...
    artifact, _ = publish_sealed_json(atoms_path.with_name(f"conditional-method-prefix-{limit:04d}.json"), {
        "format": "memory-condense-conditional-source-admission-verification-v1",
...
def load_verified_method(atoms_path, atoms):
    ...
    receipt = read_sealed_json(atoms_path.with_name(f"conditional-method-prefix-{match[1]}.json"))
    p = receipt.payload
