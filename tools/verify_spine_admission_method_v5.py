"""Verify conditional admission including support-array opening quote repair."""
import argparse
import json
...
from tools.admit_spine_corpus_v4 import admit
from tools.execute_spine_corpus import prepare
from tools.matched_eval.artifacts import publish_sealed_json, read_sealed_json
from tools.repair_spine_summary_budget_v3 import IMPLEMENTATION as REPAIR_IMPLEMENTATION, batches_for
from tools.spine_transport_lineage import verify_transport_lineage


ADMISSION_IMPLEMENTATION = ("tools/admit_spine_corpus_v4.py",
...
IMPLEMENTATION = tuple(dict.fromkeys((*base.IMPLEMENTATION, *ADMISSION_IMPLEMENTATION,
    *REPAIR_IMPLEMENTATION, "tools/verify_spine_admission_method_v5.py")))
SYNTAX_RULE = "missing or misplaced support-string terminators, or duplicated support-array opening quote; preserve original response and every summary byte"
LEGACY_SYNTAX_RULE = "missing or misplaced support-string terminators only; preserve original response and every summary byte"


def conditional_method(policy):
    normalized = policy
    if policy.get("format") == "memory-condense-spine-source-binding-policy-v7":
        if (policy.get("syntax_repair") != SYNTAX_RULE or
                policy.get("summary_compaction_batching") != base.base.BATCHING or
                policy.get("implementation") != base.base.legacy.hashes(ADMISSION_IMPLEMENTATION)):
            raise ValueError("unrecognized support syntax admission rule")
        normalized = dict(policy)
        normalized["syntax_repair"] = LEGACY_SYNTAX_RULE
        if policy["summary_budget_repairs_sha256"] is not None:
            normalized["format"] = "memory-condense-spine-source-binding-policy-v5"
            normalized["implementation"] = base.base.legacy.hashes(base.base.ADMISSION_IMPLEMENTATION)
        else:
            normalized.pop("summary_compaction_batching")
            normalized["format"] = "memory-condense-spine-source-binding-policy-v3"
            normalized["implementation"] = base.base.legacy.hashes(base.base.legacy.ADMISSION_IMPLEMENTATION)
    method = base.conditional_method(normalized)
    method["format"] = "memory-condense-conditional-source-admission-method-v5"
    method["syntax_repair"] = SYNTAX_RULE
    ...
    if policy.payload["format"] != "memory-condense-spine-source-binding-policy-v7":
        ...
        if repair_root is None:
            ...
            if (repairs.sha256 != repair_sha or repairs.payload["preflight_sha256"] != preflight.sha256 or
                    p["format"] != "memory-condense-spine-summary-budget-repair-v3" or
        ...
        payload = {"atoms_sha256": atoms.sha256, "source_binding_policy_sha256": policy.sha256,
            ...
            "additional_support_syntax_rule_needed": any(
                bool(r.get("support_json_syntax_repair", {}).get("original_removal_offsets"))
                for r in data["admission_audits"] if r.get("support_json_syntax_repair"))}
    artifact, _ = publish_sealed_json(atoms_path.with_name(f"conditional-method-v5-prefix-{limit:04d}.json"), {
        **payload, "format": "memory-condense-conditional-source-admission-verification-v5",
...
def load_verified_method(atoms_path, atoms):
    ...
    receipt = read_sealed_json(atoms_path.with_name(f"conditional-method-v5-prefix-{match[1]}.json"))
    p = receipt.payload
