"""Verify conditional admission including duplicated support delimiter repair."""
import argparse
import json
...
from tools import verify_spine_admission_method_v5 as previous
...
IMPLEMENTATION = tuple(dict.fromkeys((*previous.IMPLEMENTATION, *ADMISSION_IMPLEMENTATION,
    *REPAIR_IMPLEMENTATION, "tools/verify_spine_admission_method_v6.py")))
...
def conditional_method(policy):
    normalized = policy
    if policy.get("format") == "memory-condense-spine-source-binding-policy-v8":
        if (policy.get("syntax_repair") != SYNTAX_RULE or
            ...
            raise ValueError("unrecognized support delimiter admission rule")
        normalized = dict(policy)
        normalized["format"] = "memory-condense-spine-source-binding-policy-v7"
        normalized["syntax_repair"] = previous.SYNTAX_RULE
        normalized["implementation"] = base.base.legacy.hashes(previous.ADMISSION_IMPLEMENTATION)
    method = previous.conditional_method(normalized)
    method["format"] = "memory-condense-conditional-source-admission-method-v6"
...
def verify(atoms_path, repair_root=None):
    ...
    artifact, _ = publish_sealed_json(atoms_path.with_name(f"conditional-method-v6-prefix-{limit:04d}.json"), {
        **payload, "format": "memory-condense-conditional-source-admission-verification-v6",
...
def load_verified_method(atoms_path, atoms):
    ...
    receipt = read_sealed_json(atoms_path.with_name(f"conditional-method-v6-prefix-{match[1]}.json"))
    p = receipt.payload
